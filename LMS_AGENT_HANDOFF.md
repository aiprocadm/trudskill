...
- **Открытые follow-up'ы (подтверждены чтением, НЕ исправлены — отдельные TDD-задачи; #1–#5 закрыты выше):** (6) **HIGH-ish** идемпотентность bulk: кэш отдаёт устаревший outcome при повторе ключа с другим телом (нет request-hash) — спорно (стандартный контракт идемпотентности; менять поведение рискованно); (7) **LOW** integrations webhook: timing-unsafe сравнение + отключение проверки при пустом секрете (в проде секрет обязателен); (8) **LOW** мёртвый дублёр-грейдер `calculateAttemptScore` со слабой логикой.
