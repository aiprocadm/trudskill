...
- **Тест-статус (финальные прогоны):** `pnpm --filter @cdoprof/ui exec vitest run` — **15/15** (новые `visually-hidden` 4 + `status-label` 2 + existing foundation/smoke 9); `pnpm test:frontend` — **546/546** (в холодном полном прогоне 6 dynamic-import-smoke e2e упали по таймауту 5000ms из-за холодного vite-transform на кириллице ~46s — НЕ логика; при изолированном тёплом прогоне все 6 зелёные 88/88, ~300ms каждый); `pnpm lint` — **8/8** (a11y-гейт чист `--max-warnings=0`; остаются только 2 pre-existing react-hooks _warning_ в test-attempt-screen, не a11y, `next lint` на warning не падает); `pnpm typecheck` — **8/8**. Полный backend-suite не гоняли (вне скоупа трека + Windows/Cyrillic краш).
- **Отклонения/решения:** label-паттерн = visually-hidden `<label>` (D-B2, не `aria-label`); гейт = recommended (не strict); тест-файлы a11y созданы как `.tsx` (vitest-config ui `include: ['src/**/*.test.tsx']` — `.ts` не подхватываются); номер handoff = **5.122** (как в плане; параллельный Track A претендует на 5.121, Track C может претендовать на 5.122 — конфликт docs разрешит оркестратор при мердже).
- **Следующий шаг:** PR этой ветки (создаст оркестратор) → merge после ревью Track A. Отложено (вне скоупа, зафиксировано): runtime axe-аудит (нет DOM-окружения); субтитры к видео (отдельная content-authoring-фича).
