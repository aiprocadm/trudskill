...
### 5.131 Phase 6 — проброс `signatureStatus` end-to-end + fake staging document-signer (НЭП)

- **Контекст:** активация одного из follow-up-остатков §5.129 на уровне кода — поля подписи писались на `GeneratedDocumentEntity`, но никем не читались. Ветка `feat/2026-06-17-document-signature-status-passthrough`, **PR #259** (отдельно от КЭП-выгрузок §5.132).
- **Что сделано:** `LearnerDocumentDto.signatureStatus` (тип `DocumentSignatureStatus`) + бейдж «Подписана НЭП»/«Ошибка подписи» в кабинете слушателя (plain `<span>`, НЕ StatusChip — его словарь мапит `signed` неверно); `PublicVerifyResult.signatureStatus?: 'signed'` (сужено — `failed`/`unsigned` не утекают на публичную QR-проверку); `FakeDocumentSignatureProvider` (staging-превью, синтетический `signed`, self-маркировка) + env-refinement запрещает `ESIGN_PROVIDER=fake` в `NODE_ENV=production` (staging разрешён). Без миграции (0053), без новых прав. backend 65/65, frontend 9/9. (Подробности — в memory `project_esign_phase6.md`.)

### 5.132 Phase 6 — КЭП-подпись файлов выгрузок (provider-agnostic seam, dormant)

- **Контекст:** вторая половина гибридной модели владельца (НЭП на документы + **КЭП на выгрузки** в госреестры). До этого 5 экспортёров (ФРДО/ОТ/ЕИСОТ/Ростехнадзор/НМО) отдавали **неподписанные** XLSX. Полный цикл brainstorming → spec → plan (9 задач) → subagent-driven (4+4 параллельных субагента на интеграционные этапы) + inline-эталон. Ветка `feat/2026-06-17-export-kep-signature-seam` (от `origin/main`, **не** содержит §5.130/PR #259). Спек [`docs/superpowers/specs/2026-06-17-export-kep-signature-seam-design.md`](docs/superpowers/specs/2026-06-17-export-kep-signature-seam-design.md), план [`docs/superpowers/plans/2026-06-17-export-kep-signature-seam.md`](docs/superpowers/plans/2026-06-17-export-kep-signature-seam.md). Зеркалит document-signature seam (§5.129) и AV-паттерн, но семантика — **detached** `.p7s` (CMS/PKCS#7) над байтами файла (не встроенный PDF-штамп НЭП).
...
- **Находки/решения:** (1) Память про §5.131 «fake document-провайдер» оказалась **неверной** — в коде `ESIGN_PROVIDER` = `enum(['noop','cryptopro'])` без fake/prod-guard; export-seam добавил `fake`+guard **заново** (осознанно, для staging-превью пайплайна+бейджа). (2) Найден **пред-существующий баг на main**: тест `env.test.ts` «parses production env…» красный, т.к. PR #258 (ЕСИА) добавил strict-проверку `ESIA_STATE_SECRET` без обновления фикстуры `strictValidEnv` → заведён follow-up на отдельной ветке `fix/2026-06-17-env-test-esia-state-secret` (коммит `e13568c`, изолированно от этой фичи). Мой `env.export-sign.test.ts` обходит это собственной prod-фикстурой с non-dev `ESIA_STATE_SECRET`. (3) `outcome.signatureStatus` намеренно несёт и `'unsigned'` (стамп ставится всегда при наличии файла) — бейдж всё равно даёт `null` для unsigned.
