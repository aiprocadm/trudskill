...
**Дальше по этому ТЗ:** незакрытыми остаются `C2` («личное дело»), `B2`/`B3` (защищённое
воспроизведение и учёт часов — ждут решения владельца по видеопровайдеру), `G3` (двухфакторный
вход), `A1` (движок рендера — инфраструктура Gotenberg готова).
