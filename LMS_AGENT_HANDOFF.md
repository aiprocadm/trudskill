...
### 5.168 Фаза 0 «Фундамент», Task 3 — контейнер ClamAV + прод-включение (ФТ-G5) + фикс протокола INSTREAM

- **Контекст:** продолжение Фазы 0 по плану [2026-07-23-tz-faza0-fundament.md](docs/superpowers/plans/2026-07-23-tz-faza0-fundament.md); после Task 1 (§5.167) следующий по порядку — Task 3.
- **Summary:** сервис `clamav` (`clamav/clamav:1.4`, healthcheck `clamdcheck.sh`, volume под базы сигнатур, `start_period: 300s` — первый старт качает ~300 МБ) добавлен в **dev-compose** (порт 3310, backend от него НЕ зависит — dev остаётся `ANTIVIRUS_ENABLED=false` → Noop) и в **prod-compose** (порт наружу не публикуется; backend получает `depends_on: clamav: service_healthy` и принудительный `environment: ANTIVIRUS_ENABLED=true, CLAMAV_HOST=clamav, CLAMAV_PORT=3310` поверх `.env.production`). **Живой дымовой тест вскрыл протокольный баг сканера:** clamd на команду отвечал `UNKNOWN COMMAND` — z-команды clamd должны быть NUL-терминированы, а код слал `'zINSTREAM '` (пробел); плюс ответы clamd NUL-терминированы (`stream: OK\0`), а `parseReply` NUL не срезал (`trim()` его не берёт) → даже при верной команде вердикт был бы `error`. Итог до фикса: с включённым флагом **все** загрузки отбивались бы (fail-closed). Починено: `'zINSTREAM\0'` + срез NUL в `parseReply`; юнит-симулятор clamd приведён к реальному поведению (NUL-терминированные ответы). Комментарий «verify against a real clamd» в сканере закрыт.
- **Files changed:** `infra/docker-compose.yml`, `infra/docker-compose.prod.yml`, `infra/README.md`, `apps/backend/src/infrastructure/antivirus/clamav-antivirus.scanner.ts`, `apps/backend/src/infrastructure/antivirus/clamav-antivirus.scanner.test.ts`, `docs/LAUNCH_CHECKLIST.md` (пункт D2 про антивирус закрыт), `docs/TZ_ARENDNAYA_SDO_STATUS.md` (G5 → ✅), план Фазы 0, `README.md`, `LMS_AGENT_HANDOFF.md`. `.env.example`/`env.schema.ts` не тронуты — переменные там уже были.
- **Тесты:** живой smoke на тестовом сервере — контейнер из нового dev-compose дошёл до `healthy`, реальный `ClamAvAntivirusScanner` против него: чистый файл → `clean`, EICAR → `infected: Eicar-Test-Signature`. Юнит: `vitest run src/infrastructure/antivirus` — 2 файла / 5 тестов зелёные. `docker compose config` обоих файлов чист. Полный `ci:check` не гонялся (изменения точечные: compose + 2 файла антивируса + docs).
- **Deviations:** «код сканера не тронут» из плана нарушено осознанно — без фикса протокола acceptance «EICAR отбивается» недостижим (см. Summary). Дымовой тест сделан против живого clamd реальным классом сканера, а не полным boot'ом prod-профиля (сборка всех образов на тестовом сервере избыточна для инфра-задачи).
- **Next (Фаза 0):** Task 6 (Gotenberg-контейнер) → Task 2 (rate limiting на `/verify/{qr}`) → Task 4 (email-события) → Task 5 (2FA TOTP) → Task 7 (шифрование ПДн). Каждая — отдельный под-PR.
