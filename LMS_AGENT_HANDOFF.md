...
### 5.147 Phase 5C-2 — валидация формата email на бэкенд-границе (staff-получатели)

- **Контекст:** продолжение ревью-прохода «улучшай код, без логических ошибок». Найден реальный пробел trust-boundary: `SetNotificationStaffRecipientsRequest` (PUT `/notification-staff-recipients`) валидировал адреса лишь как `@IsString` + `@MaxLength(254)` — формат email **не** проверялся. Фронтенд (`notification-recipients/screens.tsx`) проверяет `EMAIL_RE` и блокирует «Сохранить», но любой запрос в обход UI (curl/скрипт) сохранил бы мусорную строку в MVP-снимок (`notificationStaffRecipients`), после чего ночной `RemindersSchedulerService` пытался бы слать на неё через все три сканера (recert/deadline/license-expiry). По CLAUDE.md бэкенд — граница доверия («never trust `@Body()` directly»).
- **Применённое улучшение:** добавлен `@IsEmail({}, { each: true })` в DTO. Нормализация (trim/lowercase/дедуп) в `MvpService.setNotificationStaffRecipients` сохранена — `@IsEmail` отсекает формат до неё; фронтенд всегда шлёт trimmed-значения, так что пользователи не затронуты. Очистка списка (`emails: []`) по-прежнему проходит (each-валидатор на пустом массиве — ноль ошибок).
- **Тест-статус:** новый `notification-recipients.dto-validation.test.ts` (**6** кейсов: валидные / пустой массив = выключить / невалидный email / не-строка / не-массив / >50 cap) — TDD (кейс невалидного email падал до фикса, затем зелёный). Регресс staff-recipients service + scanners + reminder-recipients **22** зелёные; backend typecheck чисто; ESLint `--max-warnings=0` чисто. HTTP-интеграционный stub-тест не затронут (использует валидный `admin@uc.ru`, stub-контроллер не вызывает `assertValidDto`). **Без миграции/прав/изменения хранимой формы.**
