...
- **Намеренно отложено (follow-up'ы, задокументированы как spawn-задачи):** frontend cannot-unset (program-meta / commission description — нужен cross-layer null-clear), payment fulfillment retry dead-end (идемпотентный replay упавших слушателей), esign group-order дубль-сертификат vs auto-issue, ~~провизорный `ExamResult` `final/passed` до ручной проверки эссе~~ (закрыт в §5.155), in-memory `resolvePermissions` мёртвый conditional (non-prod, LOW), `GroupCourse.courseVersionId` pinning не подключён end-to-end.

### 5.155 Закрыт отложенный follow-up §5.154 — провизорный `ExamResult` `final/passed` до ручной проверки эссе (MEDIUM)
...
- **TDD:** +5 тестов (`§5.155` в `mvp.service.test.ts`): провизорный submit не публикует pass; `needs_review`→pass после review; auto-only регресс-гард; гейт модуля заперт во время review и открывается после; аналитика не считает provisional за pass. RED подтверждён до фикса.
...
- **Ветка:** `fix/exam-provisional-pass-5155` (worktree `C:/Users/karka/ts-exam-wt`), коммит `2c04344` — отдельная ветка от §5.154 (HEAD), для merge в основную.
