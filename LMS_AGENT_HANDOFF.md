...
- Date: 2026-08-18 (UTC+3), последняя запись — §5.301 (фаза 7 редизайна завершена, PR #536)
- Agent: Claude Code
...
- **Актуальный срез (2026-08-18, §5.301, PR #536): ФАЗА 7 ЗАВЕРШЕНА** — категория A (письма/TOTP/SMTP_FROM/OpenAPI на trudskill) + категория D (базовое ТЗ → `TZ_TRUDSKILL_BASE.md`, `MET-004`, живые заголовки). Открытые вопросы владельцу: реальный почтовый домен (BR-032), уведомление о новом имени TOTP. Дальше — фаза 8 (окно обслуживания): `BR-020`–`BR-023` ключи двойным чтением + две выкатки с интервалом 60 дней; `BR-031`/C-2 — последними после учений.
