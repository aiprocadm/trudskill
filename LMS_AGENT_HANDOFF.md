...
- **Отложено (требует отдельного плана):** `license_expiring` — нужна миграция лицензий из in-memory (`InMemoryOrgState`) в postgres, чтобы cross-tenant cron-сканер имел durable-источник; это самостоятельная архитектурная работа, не «хвост».
