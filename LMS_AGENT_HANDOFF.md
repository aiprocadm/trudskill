...
- **Тест-статус:** worker 15/15 (typecheck+lint), backend целевые сьюты — test-player 13, mvp.service 100, mvp.concurrency 2, business-flows.e2e 4, ot-registry 34, **documents+esign 300** (25 файлов, +1 roster-тест); монорепо **typecheck 8/8**, ESLint по изменённым файлам чисто. **Без миграций (0057), без новых прав, без изменения API-конвертов.**
- **Открытые follow-up'ы (подтверждены чтением, НЕ исправлены — отдельные TDD-задачи; #1 esign-«кирпич», #2 finalize-идемпотентность, #3 roster-guard закрыты выше):** (4) **MEDIUM** assessment: два финализатора пишут разные поля (`finalScore` vs `bestScore`) в один `ExamResult` (impact смягчён fallback'ом `bestScore ?? finalScore`); (5) **MEDIUM** bulk-import: две строки, резолвящиеся в одного существующего слушателя по разным полям (email vs СНИЛС), затирают друг друга в `learnerIdToRowNumber` → расхождение счётчиков; (6) **HIGH-ish** идемпотентность bulk: кэш отдаёт устаревший outcome при повторе ключа с другим телом (нет request-hash) — спорно (стандартный контракт идемпотентности); (7) **LOW** integrations webhook: timing-unsafe сравнение + отключение проверки при пустом секрете (в проде секрет обязателен); (8) **LOW** мёртвый дублёр-грейдер `calculateAttemptScore` со слабой логикой.
