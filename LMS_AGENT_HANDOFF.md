...
**Как исправлено (и почему именно так):** ручка переведена на `courses.read`, а **наполнение разделов гейтится по правам актора внутри сервиса**: сроки и группы — по `enrollments.read`, очередь проверки — по `assessment.reviews.review`, пробелы в программах — по `courses.read`. Без права на зачисления пробелы считаются **по курсам, без названий групп** — иначе состав обучения утёк бы тому, кому его видеть не разрешено. Скрытые разделы перечисляются в `hiddenSections` и показываются на экране как «нет доступа», а не исчезают: пропавший блок читается как поломка. Так один адрес честно работает и для методиста, и для менеджера, и для администратора.
...
**Тесты:** бэкенд **2949** (+20: 11 на сборку, 6 на гейтинг разделов по правам, 3 на HTTP-границу), фронт **877**. `ci:check` exit 0.
