...
### 5.122 Зелёный прогон тестов — починка регрессии explicit-`@Inject` DI (накопилась с Phase 5)

- **Запрос:** `/goal` «продолжай до тех пор пока все тесты не станут зелеными, не будет падений» на ветке `feat/2026-06-13-phase-10a-excel-report-builder`.
- **Найденная реальная поломка:** `apps/backend/src/common/di-explicit-injection.test.ts` (страж из #236 — под `tsx`/esbuild нет `emitDecoratorMetadata`, поэтому каждый инжектируемый параметр конструктора ОБЯЗАН иметь явный `@Inject(Token)`, иначе `NestFactory.create()` зависает на старте) падал с **14 нарушителями**. Все — НЕ из report-builder, а накопились начиная с Phase 5 (reminders/recertification) и Phase 0 (email-sender); проскочили потому что полный backend-suite локально крашится (Windows+Cyrillic), а отдельные unit-тесты `new`-ают классы напрямую и страж в CI, видимо, маскировался. Идентично на `main` (ветка эти файлы не трогала).
- **Фикс:** добавлен явный `@Inject(<Class>)` к 13 параметрам в 6 провайдерах: `mvp/infrastructure/mvp-tenant-runner.service.ts` (TenantSerialGateway), `mvp/recertification/recertification-scanner.service.ts` (NotificationDispatcher, DocumentsTenantRunner), `mvp/recertification/recertification.service.ts` (MvpService, RecertificationScanner), `mvp/reminders/course-deadline-scanner.service.ts` (NotificationDispatcher), `mvp/reminders/document-revoked-email.listener.ts` (MvpTenantRunner, NotificationDispatcher — плюс `import type`→value), `mvp/reminders/reminders-scheduler.service.ts` (TenantService, MvpTenantRunner, RecertificationScanner, CourseDeadlineScanner, DatabaseService). Изменения **строго аддитивные** (явные токены для уже-инжектируемых по типу зависимостей — граф DI не меняется). 14-й «нарушитель» `iam/services/email-magic-link-email-sender.ts` — **ложноположительный** (инстанцируется фабрикой `new` в `magic-link-email-sender.factory.ts`); добавлен в `FACTORY_INSTANTIATED` стража, а не аннотирован.
- **Тест-статус (изолированные прогоны по модулям, обход Cyrillic-краша):** contracts **7** ✓, frontend **560** ✓; backend по всем модулям зелёный — common+infra+seeds+reminders+recertification **134**, iam+documents **289** (минус docker-тест ниже), integrations/org/esign/communication/workspace/health/audit/tenant/files **231**, migration **3**, mvp registries **47**, mvp identity/proctoring/scorm/infra **170**, mvp assessment/pdf/reviewer/test-player/pre-exam **138**, mvp migrations+core **291**, report-builder **118**, canonical E2E **10**. `pnpm typecheck` **8/8** ✓, `pnpm lint` **8/8** ✓.
- **Docker-тест теперь грациозно скипается локально (правка, решение владельца «прими самое эффективное»):** `iam/services/postgres-magic-link-token-repo.integration.test.ts` (единственный Testcontainers-тест) оборачивается в `describe.skipIf(!isDockerAvailable())`. Новый синхронный пробник `isDockerAvailable()` в `testing/with-test-db.ts` (Windows named pipe `\\.\pipe\docker_engine` / unix `/var/run/docker.sock` / `DOCKER_HOST`) → в CI (Ubuntu, сокет есть) тест идёт полностью, на dev-боксе без Docker **скипается видимо** (vitest `↓ 7 skipped`), а не валит локальный backend-прогон. Причина недоступности здесь: Docker Desktop GUI запущен, но `com.docker.service` остановлен и не стартует без админ-прав/UAC, `docker-desktop` WSL-дистро без `dockerd`.
- **Один тест по-прежнему НЕ запускается локально (зелёный в CI на Ubuntu, не связан с правкой, скип-хак НЕ добавлялся намеренно):** `mvp/mvp.domains.http.integration.test.ts` — нативный краш воркера от Cyrillic-пути (`ERR_IPC_CHANNEL_CLOSED` + native stack, документированный в CLAUDE.md как «most consistent crash candidate»). Per-file skip бессмысленен: полный параллельный `pnpm test:backend` всё равно крашится на teardown 165 файлов, поэтому скип убрал бы покрытие, не починив полный прогон. Правильное лечение — ASCII-путь репозитория или CI.
