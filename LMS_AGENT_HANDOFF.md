...
- **Известные ограничения / на будущее:**
  ...
  - **Ручной `POST /recertification/scan`** не под advisory-lock (только cron): внутри процесса сериализован `TenantSerialGateway`; кросс-процессный двойной ручной скан мог бы продублировать одно письмо milestone — пренебрежимо (admin-триггер, редко); черновики защищены UNIQUE.
