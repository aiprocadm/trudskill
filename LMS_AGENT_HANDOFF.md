...
### Issue 5: `di-explicit-injection.test.ts` красный на ветке — пред-существующие срабатывания (не блокирует boot)

- Severity: **low** (приложение бутится; не блокирует пилот). Обнаружено при §5.137 (запуск стека), правкой §5.137 НЕ вызвано.
- Area: backend/tests (static DI-guard)
- Description: статический сканер [di-explicit-injection.test.ts](apps/backend/src/common/di-explicit-injection.test.ts) (Issue 3) флагует 4 офендера: `infrastructure/esia/esia-oidc.provider.ts` (`cfg: EsiaOidcConfig`), `infrastructure/esia/mock-esia.provider.ts` (`defaultIdentity: ...` — параметр со **значением по умолчанию**, не DI), `payments.state-machine.ts` ×2 (`from`/`to: OrderStatus` — это **не** конструктор DI-провайдера, а сигнатура value/функции, которую наивный парсер `firstCtorParams` принял за конструктор). Реальный boot прошёл успешно с обоими модулями → для дефолтной (дормант-ЕСИА) конфигурации это **ложные срабатывания**; для `esia/*` риск касается только включённого ЕСИА (`ESIA_ENABLED=true`).
- Evidence: §5.137 — `pnpm dev:stack` поднялся (`Nest application successfully started`, ready-чек зелёный) при этих же файлах; изолированный прогон `di-explicit-injection.test.ts` красный с указанными 4 офендерами.
- Suggested fix: либо (a) для `esia/*` providers добавить явный `@Inject(<Token>)`/занести в `FACTORY_INSTANTIATED`, исключить параметры со значением по умолчанию; (b) для `payments.state-machine.ts` — уточнить парсер сканера, чтобы он брал конструктор класса с DI-декоратором `@Injectable`, а не первую попавшуюся скобку `constructor(`/сигнатуру. Не входит в scope §5.137 (затрагивает дормант-ЕСИА и сам сканер).
