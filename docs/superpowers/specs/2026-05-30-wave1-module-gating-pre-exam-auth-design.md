...
- **План 1 — «Модульный гейтинг + время на изучение» (A+B).** Вместе: оба гейтят `startAttempt`, оба модуле-центричны, общий UI course-viewer + интеграция экзамена в просмотр курса. Без внешних зависимостей. ≈1 план (10–13 задач).
- **План 2 — «Аутентификация перед экзаменом №816» (C).** Таблица токенов + сервис + 2 endpoint'а + гейт в `startAttempt` + интерстишал + маркер. Зависимость от email-адаптера — флаг. ≈1 план.

Рекомендация: начать с Плана 1 (полностью автономен, быстрый результат), затем План 2.
