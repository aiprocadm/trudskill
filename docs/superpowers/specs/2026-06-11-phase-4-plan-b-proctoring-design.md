...
5. **Порядок старта**: сессия записи создаётся ДО попытки (`POST /proctoring-recordings`, требует `consent: true`, проверяет что прокторинг действительно требуется для этого learner+group+course); гейт пропускает при наличии активной записи (`recordingStatus: 'recording'`) этого ученика для этого group+course; `startAttempt` линкует `recording.attemptId = attempt.id`.
...
8. **Resume после refresh**: `GET /proctoring-recordings/active?enrollmentId&courseId` возвращает активную сессию + максимальный загруженный `sequence`; фронт продолжает с `max+1` (старый MediaRecorder умер — новый сегмент начинается с нового заголовка webm; плеер склеивает по порядку, разрыв допустим и виден админу).
...
12. **Миграция 0051**: `learning.proctoring_recordings` + `learning.group_courses.requires_proctoring` + `learning.enrollments.proctoring_override` + права и role-grants (методист → read; learner → submit; platform_admin/tenant_admin → оба; роли «teacher» в seed 0050 нет — грантим только существующим ролям). Зеркало 0050.
...
interface ProctoringRecording extends BaseEntity {
  learnerId: string;
  groupId: string; // выводится сервером из enrollmentId
  courseId: string;
  recordingStatus: ProctoringRecordingStatus; // НЕ `status` — он занят BaseEntity (та же коррекция, что verificationStatus в Plan A)
...
| --------------------------------------------------------- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- | ----------------------------------- |
| `POST /proctoring-recordings`                             | `proctoring.submit` | старт сессии: `{ enrollmentId, courseId, consent: true }` (`groupId` выводится сервером — фронт знает только enrollmentId+courseId из `LearnerTestSummary`) → запись `recording`; 400 если прокторинг не требуется или consent ≠ true; повторный старт при активной сессии → 200 идемпотентный reuse той же записи (упрощает resume) |
| `POST /proctoring-recordings/:id/chunk-upload-intent`     | `proctoring.submit` | `{ sequence, originalName, contentType, sizeBytes }` (имена полей — конвенция files-слоя `UploadIntentInput`) → presigned PUT + регистрация чанка; только своя сессия, только `recording`; дубликат sequence → 409                                                                                                                   |
| `POST /proctoring-recordings/:id/complete`                | `proctoring.submit` | стоп: `completedAt`, идемпотентно                                                                                                                                                                                                                                                                                                    |
| `GET /proctoring-recordings/active?enrollmentId&courseId` | `proctoring.submit` | активная сессия текущего ученика + `nextSequence` (resume)                                                                                                                                                                                                                                                                           |
...
- **Интерстициал в test-player** (`tests-list-screen.tsx`): регекс `/proctoring_required|video recording must be active/i` на ошибке старта (`useStartAttempt` пробрасывает только `err.message`, не code — потому матчим текст сообщения, как Plan A) → экран согласия: предпросмотр камеры, 152-ФЗ чекбокс, кнопка «Начать запись и экзамен» → старт сессии → MediaRecorder → повторный старт попытки. Индикатор записи (`● REC`) на время теста; стоп + complete после сабмита попытки.
