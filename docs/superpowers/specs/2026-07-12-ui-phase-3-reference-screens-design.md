...
## 14. Отклонения от плана (факт реализации, §5.164)

- **Объём сужен по YAGNI** (зафиксировано на старте плана): `Drawer` (существующий `Dialog` уже с focus-trap покрывает edit), `Hero`/`DashboardTile` (уровень приложения / `next/link`; каноном остаётся `NextStepCard`+`.ui-hero`), дедуп каталогов плиток и `PageContainer` — вынесены за Фазу 3.
- **`SelectField` вместо нового `Field`:** примитив поля — существующий `FormField`/`TextareaField`; добавлен только недостающий `SelectField` (зеркало `FormField`).
- **`vitest.config.ts` → `esbuild.jsx:'automatic'`** — потребовалось для тестов-как-функция (совпадает с прод-трансформом Next и `packages/ui`); прод не затронут.
- **Пилот формы** сохранил инлайн `max-width:560` на `<Form>` (общий `.ui-form` в `foundation.ts` используется 5 др. формами — глобальная правка задела бы их).
- **Пилот dashboard** адаптирован к реальной форме хука (`{ data, loading, error: string }`), строковая ошибка обёрнута в `Error` для `AsyncSection`.
- **a11y-follow-up → Фаза 6:** accessible-name folding у вложенного `.ui-field` (`SelectField` наследует от `FormField`) — чинить консистентно в a11y-проходе.
- **Гейты:** ui 68 · frontend 679 · typecheck 8/8 · ESLint чисто. Adversarial-ревью: 2 находки, 0 подтверждённых.
