...
- [x] **Step 2: Подключить в корневой flat-config.** В `eslint.config.mjs` импортировать плагин и добавить НОВЫЙ блок ПОСЛЕ next-блока и ПЕРЕД `prettier` (порядок важен: prettier должен оставаться последним). Заскоупить на jsx/tsx фронта и ui:
...
- [x] **Step 3: Совместимость с `next lint`.** `apps/frontend` линтуется через `next lint`, который читает `apps/frontend/.eslintrc.json` (legacy-формат). Чтобы статический гейт работал и в `next lint`, расширить shim:
...
- [x] **Step 4: Зафиксировать baseline нарушений (НЕ чинить здесь).** Прогнать полный линт и ЗАПИСАТЬ список:
...
- [x] **Step 1: Разобрать `/tmp/a11y-baseline.txt`.** Разделить нарушения на две группы: **(A) целевые** (в примитивах из этого плана — чиним в Task 3–10) и **(B) не-целевые** (pre-existing в экранах/виджетах, вне скоупа фиксов).
- [x] **Step 2: Для группы (B)** — НЕ выключать правило глобально. Выбрать минимально-инвазивный из двух:
...
- [x] **Step 3:** Если группа (B) ПУСТА (recommended оказался чистым вне примитивов) — этот таск no-op, отметить в handoff «baseline clean, без override» и пропустить коммит.
- [x] **Step 4: Проверка** — `pnpm lint` теперь падает ТОЛЬКО на целевых примитивах (которые починим дальше) либо чисто (если их ещё не трогали — они могут не давать ошибок recommended, тогда фиксы Task 3–10 — это семантические улучшения сверх плагина, а не lint-fix).
- [x] **Step 5: Commit (если были изменения)**
...
- [x] **Step 1: Failing test** для генератора стабильного id и константы класса:
...
- [x] **Step 4: CSS-класс.** В глобальном CSS пакета `ui` (найти существующий файл со стилями `.ui-*`) добавить стандартный visually-hidden-сниппет:
...
- [x] **Step 5: Run tests** → PASS; `npx eslint packages/ui/src/a11y/visually-hidden.ts --max-warnings=0` чисто.
...
- [x] **Step 1: Failing test** для текст-маппинга (источник доступного label, чтобы не было color-only):
...
- [x] **Step 3: Реализация** `status-label.ts` — маппинг известных `EntityStatus` на рус. подписи (свериться со значениями `EntityStatus` в `@cdoprof/shared-types`), fallback = сам ключ.

- [x] **Step 4: Фикс `badges/index.tsx`** — добавить НЕ-цветовой носитель смысла: видимый текст уже есть (`{label ?? status}`), но цвет-фон единственный различитель состояния. Добавить `role="status"`-нейтрально НЕ нужно; нужно `aria-label`/`title`, гарантирующий смысл независимо от цвета, и НЕ полагаться на фон:
...
- [x] **Step 5: Run tests + lint** → PASS, `npx eslint packages/ui/src/components/badges/ --max-warnings=0` чисто.
...
- [x] **Step 1: Фикс `LoadingState`** — добавить live-region семантику:
...
- [x] **Step 2: EmptyState** — оставить визуал, но дать `role="status"` (не alert — это не ошибка), чтобы динамическое появление «нет данных» озвучивалось:
...
- [x] **Step 3: Проверка** — `pnpm --filter @cdoprof/ui exec vitest run --no-file-parallelism` (если есть тесты states) + `npx eslint packages/ui/src/components/states/ --max-warnings=0` + `pnpm typecheck`. Чистая правка разметки, юнита на DOM нет (конвенция). Acceptance — lint+typecheck зелёные.
...
- [x] **Step 1: SearchInput** — добавить опциональный `label` (default «Поиск») через `.ui-visually-hidden`-`<label>` + связку id (паттерн D-B2). `exactOptionalPropertyTypes` — `label` со значением по умолчанию, без `undefined`-присвоений:
...
- [x] **Step 2: LookupSelect** — тот же паттерн (опциональный `label`, default разумный, например «Выбор»):
...
- [x] **Step 3: Проверка** — `npx eslint packages/ui/src/components/search/ packages/ui/src/components/select/ --max-warnings=0` (`jsx-a11y/label-has-associated-control` теперь доволен) + `pnpm typecheck`.
...
- [x] **Step 1: Фикс** — обернуть в `<nav aria-label>`, добавить `type="button"` + `aria-label` на prev/next, индикатор страницы — `aria-live="polite"`:
...
- [x] **Step 2: Проверка** — `npx eslint packages/ui/src/components/pagination/ --max-warnings=0` + `pnpm typecheck`.
...
- [x] **Step 1: FilterBar** — дать группе доступное имя (`role="group"` + `aria-label`), опциональный проп:
...
- [x] **Step 2: DataTable стабильные ключи строк.** Сейчас `rows.map((r, i) => <tr key={i}>)` — нестабильно при сортировке/фильтрации (React reconciliation + a11y focus drift). Ввести устойчивый ключ: добавить опциональный проп `rowKey?: (row: T, index: number) => string | number` с дефолтом, который пытается взять `r.id`/`r.key`, иначе индекс:
...
- [x] **Step 3: Проверка** — `npx eslint packages/ui/src/components/filters/ packages/ui/src/components/table/ --max-warnings=0` + `pnpm typecheck` + прогон существующих ui-тестов, если затрагивают DataTable (`Glob packages/ui/src/**/table*.test.ts`).
...
- [x] **Step 1: Фикс `FormField`** — сейчас error/hint визуально есть, но НЕ связаны с input через `aria-describedby`, поэтому скринридер их не озвучивает. Сгенерировать id для hint/error и связать:
...
- [x] **Step 2: TextareaField** — та же связка для `<textarea>` (id hint/error + `aria-describedby` conditional spread + `role="alert"` на error).

- [x] **Step 3: Проверка** — `npx eslint packages/ui/src/components/forms/ --max-warnings=0` + `pnpm typecheck`.
...
- [x] **Step 2: Проверка** — `npx eslint apps/frontend/src/widgets/shell/app-shell.tsx --max-warnings=0` + `pnpm typecheck`. Существующие shell e2e (`Glob apps/frontend/src/e2e/*shell*`/`*navigation*`) должны остаться зелёными — изменение чисто атрибутивное.
...
- [x] **Step 1: Полный frontend-набор** (на этой машине работает):
...
- [x] **Step 2: UI-пакет тесты** (новые юниты + existing):
...
- [x] **Step 3: Полный lint** — статический a11y-гейт зелёный по фронту и ui:
...
- [x] **Step 4: Typecheck монорепо** — `pnpm typecheck` → 8/8 PASS.

- [x] **Step 5:** При желании — `npx eslint packages/ui/src --max-warnings=0` и `cd apps/frontend && pnpm lint` отдельно, чтобы подтвердить, что плагин активен в ОБОИХ путях линтования (`eslint src` для ui и `next lint` для фронта).
