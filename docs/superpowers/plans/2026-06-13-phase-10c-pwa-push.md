...
- [x] **Step 2: Env-переменные.** В `apps/backend/src/env.schema.ts` рядом с блоком `PROCTORING_VIDEO_RETENTION_*` / `SCORM_*` (≈строка 75–80) добавить:
...
- [x] **Step 3: `infra/.env.production.example`** (рядом с PROCTORING/SCORM-блоком, тем же стилем):
...
- [x] **Step 1: Тип.** В конец `mvp.types.ts` (рядом с другими Phase-типами):
...
- [x] **Step 2: State.** В `in-memory-mvp.state.ts` — импорт типа и поле:
...
- [x] **Step 3: Коллекция.** В `mvp-collections.ts` добавить `'pushSubscriptions'` в конец `MVP_COLLECTIONS` (без этого коллекция не переживёт HTTP-запрос).
...
- [x] **Step 3: Реализация** — `web-push-keys.ts`: `normalizeSubscription(raw)` → `{ endpoint, p256dh, auth }`; `isValidBrowserSubscription(raw): boolean` (endpoint — https-URL, keys.p256dh и keys.auth — непустые строки). Без зависимостей.
...
- [x] **Step 1: Failing tests** (минимум):
...
- [x] **Step 1: Интерфейс** (`web-push-sender.ts`):
...
- [x] **Step 2: Failing tests** для `WebPushSender`:
...
- [x] **Step 1: template-push-mapping (чистая функция, TDD).** `toPushNotification(rendered: EmailTemplateBody, opts?: { url?: string }): WebPushNotification` — `title = rendered.subject`, `body = первая непустая строка rendered.body` (обрезать до ~120 символов). Тесты: длинный body обрезается; пустые строки пропускаются.

- [x] **Step 2: Failing dispatcher-тесты.** Расширить `notification-dispatcher.service.test.ts` (инжектить мок `WEB_PUSH_SENDER`):
...
- [x] **Step 4: Реализация.** В `DispatchInput.recipients` тип `DispatchRecipient` + `userId?`. В конструктор `NotificationDispatcher` добавить `@Inject(WEB_PUSH_SENDER) private readonly pushSender: WebPushSenderPort`. В конце `dispatch()` (после цикла email):
...
- [x] **Step 5: Прокинуть `userId` в 5 call-sites.** В каждом listener/scanner, где формируется `recipients: [{ email, name, kind }]`, добавить `userId` (учеников — их IAM-link/userId; найти по образцу как они уже резолвят имя/email). Если у какого-то источника userId недоступен — оставить без него (push просто не уйдёт, email уйдёт). Файлы: `enrollment-email.listener.ts`, `recertification-scanner.service.ts`, `course-deadline-scanner.service.ts`, `document-revoked-email.listener.ts`.

- [x] **Step 6: Run** (dispatcher + mapping тесты, изолированно) → PASS.
...
- [x] **Step 2: Контроллер** (`@UseInterceptors(MvpRequestPersistenceInterceptor)` — т.к. пишем в MVP-state; свериться с другими MVP-контроллерами по точному набору интерсепторов):
...
- [x] **Step 3: HTTP integration** — boot минимального Nest-app со стаб-контроллером (паттерн `mvp.http.integration.test.ts`): без токена/tenant → 401/403 (TenantGuard); с валидным контекстом → 200 и операция скоупится по userId; **подтвердить, что НЕТ permission-гейта** (любой аутентифицированный tenant-user может подписаться).

- [x] **Step 4: Run** (dto + http integration, изолированно) → PASS.
...
- [x] **Step 1: Провайдеры.** В `communication.module.ts`:
...
- [x] **Step 2: Проверка** — `pnpm typecheck` + (если есть) smoke-тест модуля → PASS.
...
- [x] **Step 3: Проверка** — `pnpm --filter @cdoprof/frontend build` (или dev-build) → manifest-route компилируется, иконки на месте. ESLint по `app/manifest.ts` clean.
...
- [x] **Step 1: Установить** `pnpm --filter @cdoprof/frontend add @serwist/next serwist`.

- [x] **Step 2: `src/app/sw.ts`** — минимальный Serwist SW по доке: `Serwist` с `precacheEntries: self.__SW_MANIFEST`, `precacheOptions`, `skipWaiting`, `clientsClaim`, `navigationPreload: true`, `runtimeCaching: defaultCache`. **Push-хендлеры добавит Task 12** (этот таск — только app-shell).

- [x] **Step 3: `next.config.ts`** — импортировать `withSerwist` из `@serwist/next`, обернуть существующий `nextConfig` (сохранить текущие `rewrites` для SCORM!). Конфиг Serwist отключён в dev по умолчанию (`disable: process.env.NODE_ENV === 'development'`) — это нормально, не мешает разработке.

- [x] **Step 4: Проверка** — `pnpm --filter @cdoprof/frontend build` → SW генерируется в `public/sw.js`, нет ошибок компиляции; `pnpm typecheck` PASS (SW-типы изолированы). ESLint по `src/app/sw.ts` / `next.config.ts` clean (могут понадобиться eslint-disable для webworker-globals — минимально).
...
- [x] **Step 1: Failing tests — push-logic.test.ts** (чистые функции):
...
- [x] **Step 4: api.ts** — обёртки над `apiRequest`: `getPublicKey(session) → GET /web-push/public-key`; `subscribe(session, body) → POST /web-push/subscribe`; `unsubscribe(session, endpoint) → DELETE /web-push/subscribe`; `listSubscriptions(session) → GET /web-push/subscriptions`. Заголовок `x-tenant-id` как в других фичах.

- [x] **Step 5: api.contract.test.ts** — `vi.stubGlobal('fetch', ...)`, проверить envelope-unwrap `{data,meta}`, корректные URL/методы/body/`x-tenant-id` для всех 4 вызовов (минимум `getPublicKey` + `subscribe`).
...
- [x] **Step 1: SW push-хендлеры** в `src/app/sw.ts`:
...
- [x] **Step 2: hooks.ts — `usePushSubscription(session)`:**
...
- [x] **Step 3: PushSettingsScreen** — `SectionCard` «Push-уведомления»: если `!supported` → `SectionEmpty` («Push-уведомления недоступны: не настроены администратором или не поддерживаются браузером»); иначе тумблер «Включить push» (subscribe) / «Отключить» (unsubscribe), статус разрешения, `FieldError`/`SectionError` на ошибки. `'use client'`.

- [x] **Step 4: Страница.** Проверить существующую страницу настроек уведомлений (`Grep "settings/notifications\|email-templates" apps/frontend/app`). Встроить `PushSettingsScreen` туда; если нет — создать `app/admin/settings/notifications/page.tsx` под `<ProtectedPage>` (доступ — see Task 13).

- [x] **Step 5: Проверка** — `pnpm typecheck` + ESLint затронутых файлов + `pnpm --filter @cdoprof/frontend build` (SW с push-хендлерами компилируется). НЕ пишем render-тестов на компонент/SW (конвенция).
...
- [x] **Step 1: Навигация.** Если создавалась новая страница настроек — добавить `routeMeta` (доступ: любой аутентифицированный пользователь — push self-service; зеркалить политику доступа inbox-уведомлений, минимальную) + `navigationModel` (раздел «Настройки» / «Уведомления»). Если встроено в существующую страницу — этот шаг пропустить.

- [x] **Step 2: e2e тесты** (конвенция: НЕ render; route-access + чистые пайплайны + dynamic-import smoke):
...
- [x] **Step 3: Полный frontend-прогон** — `pnpm test:frontend` (работает на этой машине). Expected: PASS.
...
- [x] **Step 1: Backend изолированные прогоны** (полный backend-suite на Windows/Cyrillic падает — НЕ запускать целиком), каждый с `--no-file-parallelism`:
...
- [x] **Step 2: Frontend** — `pnpm test:frontend` + `pnpm --filter @cdoprof/frontend build` (PWA manifest + SW). Expected: PASS.

- [x] **Step 3: Lint + typecheck монорепо** — `pnpm typecheck` (8/8) и `pnpm lint`. Свои файлы — `npx eslint <paths> --max-warnings=0`. Pre-existing падения вне наших файлов не блокируют.

- [x] **Step 4: Manual/deferred verification (зафиксировать в handoff, НЕ автоматизируем):**
...
- [x] **Step 5: Commit** (если были мелкие фиксы по верификации; иначе пропустить).
