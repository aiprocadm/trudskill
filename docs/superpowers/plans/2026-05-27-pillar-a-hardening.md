...
- [x] **Step 1: Добавить docstring к `write()`**
...
- [x] **Step 2: Добавить docstring к `writeCritical()`**
...
- [x] **Step 3: Прогнать тесты модуля audit**
...
- [x] **Step 1: Создать файл тестов с red-test'ом**
...
- [x] **Step 2: Прогнать тест, убедиться что fail**
...
- [x] **Step 3: Обновить сигнатуру `finalizeDocument` в service**
...
- [x] **Step 5: Прогнать тест, убедиться что PASS**

Run: `pnpm --filter backend test src/modules/documents/documents.audit-completeness.test.ts`
Expected: PASS.

- [x] **Step 6: Прогнать ВСЕ тесты модуля documents**
...
- [x] **Step 1: Добавить red-test в `documents.audit-completeness.test.ts`**
...
- [x] **Step 3: Обновить `archiveDocument` в service**
...
- [x] **Step 5: Прогнать тесты модуля documents**
...
- [x] **Step 1: Добавить red-тесты в `documents.audit-completeness.test.ts`**
...
- [x] **Step 2: Прогнать тесты, убедиться что fail**
...
- [x] **Step 3: Сделать `revokeDocument` async и перевести на `writeCritical`**
...
- [x] **Step 4: Сделать `reissueDocument` async и перевести на `writeCritical`**
...
- [x] **Step 5: Сделать `issueGroupOrder` async и перевести на `writeCritical`**
...
- [x] **Step 6: Обновить контроллер (3 эндпоинта)**
...
- [x] **Step 7: Прогнать тесты модуля documents**
...
- [x] **Step 1: Red-тест в `documents.audit-completeness.test.ts`**
...
- [x] **Step 4: Обновить `activateTemplateVersion`**
...
- [x] **Step 5: Обновить controller (2 эндпоинта)**
...
- [x] **Step 6: Прогнать тесты, починить регрессии**
...
- [x] **Step 5: Прогнать тесты, починить регрессии**
...
- [x] **Step 4: Обновить controller (3 эндпоинта)**
...
- [x] **Step 3: Обновить `createTemplateBinding` (`documents.service.ts:317`)**
...
- [x] **Step 4: Обновить `updateTemplateBinding` (`:339`)**
...
- [x] **Step 5: Обновить `deleteTemplateBinding` (`:345`)**
...
- [x] **Step 6: Обновить controller (3 эндпоинта)**
...
- [x] **Step 7: Прогнать тесты, починить регрессии**
...
- [x] **Step 3: Добавить поля `ip`/`userAgent` в `DocumentGenerationTaskEntity`**
...
- [x] **Step 4: Записать `ip`/`userAgent` в task при создании**
...
- [x] **Step 5: Передать `ip`/`userAgent` из task в `writeTaskAudit`**
...
- [x] **Step 1: Создать red-тест `licenses.audit.test.ts`**
...
- [x] **Step 3: Сделать `create`/`update`/`revoke` async, перевести на `writeCritical`**
...
- [x] **Step 4: Обновить controller, если методы там вызываются синхронно**
...
- [x] **Step 5: Прогнать тесты licenses + org**
...
- [x] **Step 1: Создать red-тест `learner-pdf-card.audit.test.ts`**
...
- [x] **Step 3: Инжектировать `AuditService` в `LearnerPdfCardService`**
...
- [x] **Step 4: Изменить `composeData` на async с audit**
...
- [x] **Step 5: Обновить `mvp.controller.ts` вызов**
...
- [x] **Step 6: Обновить `mvp.module.ts` — убедиться что `AuditService` доступен**
...
- [x] **Step 1: Red-тест — изменить существующий test "writes audit entry"**
...
- [x] **Step 3: Сделать `verify` async и перевести на `writeCritical`**
...
- [x] **Step 4: Прогнать тесты public-verify**
...
- [x] **Step 5: Прогнать ВСЕ backend tests**
...
- [x] **Step 2: Создать PR-1 (через gh CLI)**
...
- [x] Smoke по существующему API — revoke / reissue / finalize не сломаны
...
- [x] **Step 3: Дождаться зелёных CI checks**
...
- [x] **Step 1: Создать red-тест (один большой файл, batch all endpoints)**
...
- [x] **Step 2: Прогнать тесты — ожидаем PASS (теория защиты подтверждается)**
...
- [x] **Step 3: Если уязвимость найдена — починить**
...
- [x] **Step 1: Создать red/green-test (зависит от того, защищён ли сервис)**
...
- [x] **Step 2: Прогнать — ожидаем PASS**
...
- [x] **Step 1: Red-тест на метаданные Throttle**
...
- [x] **Step 3: Добавить SENSITIVE_FIELDS и helper в `AuditService`**
...
- [x] **Step 4: Использовать `maskPii` в `buildRecord`**
...
- [x] **Step 5: Прогнать тесты audit**

Run: `pnpm --filter backend test src/modules/audit`
Expected: PASS — наш новый файл + существующий `audit.service.test.ts`.

- [x] **Step 6: Прогнать ВСЕ backend тесты — проверить регрессии**
...
- [x] **Step 3: Добавить `idempotencyKey` в `GenerateDocumentsBatchRequest`**
...
- [x] **Step 4: Изменить `generateDocumentsBatch` использовать `req.idempotencyKey`**
...
- [x] **Step 6: Прогнать ВСЕ тесты documents**
...
- [x] Manual smoke: revoke / reissue / batch retry / public verify работают как раньше
...
- [x] **Step 2: Проверить, что markdown валидный + ссылки работают**
...
- [x] **Step 1: Открыть `docs/operations-runbook.md`**

Read: `docs/operations-runbook.md`.

- [x] **Step 2: Найти раздел с модулями/доменами и добавить ссылку**
...
- [x] **Step 3: Проверить сценарий 2 (массовый revoke)**
...
- [x] **Step 4: Проверить сценарий 3 (QR not found)**
...
- [x] **Step 5: Проверить сценарий 4 (152-ФЗ удаление)**
...
- [x] **Step 6: Проверить сценарий 5 (компрометация — на shadow окружении)**
...
- [x] **Step 7: Записать результаты smoke в чек-лист**
...
- [x] **Step 8: Если найдены расхождения — поправить runbook**
...
- [x] **Step 9: Commit smoke-чек-листа**
...
- [x] **Step 3: Дождаться review и merge**
