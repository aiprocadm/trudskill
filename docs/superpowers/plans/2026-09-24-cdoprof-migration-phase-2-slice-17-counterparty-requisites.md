...
- [x] Типы и API (`suggestByInn`), контрактный тест.
- [x] Форма разделами, «Заполнить по ИНН» (подставляет только пустые поля), менеджер из сотрудников.
- [x] Карточка: реквизиты словами, пустые не показываются.
- [x] Документация §5.599 (МГ-D1.1–D1.2 ✅ по коду, живая подстановка 🚫 до ключа О11), `pnpm ci:check`, PR, слияние. Отклонения: подпись «Ответственный за компанию» вместо «менеджер» (Р1, РМ115); ФИО ответственного подставляет сервер в `GET /counterparties/:id`.
