...
**Выполнено:** контейнеры/env — §5.168 (PR #308); живой EICAR-smoke — §5.170. Deviations: (1) дымовой тест сделан не через полный boot prod-профиля, а против живого `clamav/clamav:1.4` из compose-сервиса **реальным классом `ClamAvAntivirusScanner`** (clean → `clean`, EICAR → `infected: Eicar-Test-Signature`); (2) «код сканера не тронут» нарушено осознанно (§5.170) — живой clamd вскрыл протокольный баг (команда `zINSTREAM` не была NUL-терминирована + ответы clamd NUL-терминированы и не парсились) → любой скан возвращал `error`, т.е. с `ANTIVIRUS_ENABLED=true` prod отбивал бы **все** загрузки. Исправлено в сканере + юнит-тесты приведены к поведению реального clamd.
...
- [x] Миграция: колонки `totp_secret_encrypted`, `totp_enabled` (шифрование секрета — тем же application-crypto, что в Task 7 / уже применяется к секретам интеграций).
- [x] `TotpService`: генерация секрета, otpauth-URL для QR, проверка кода с окном ±1.
- [x] Login-флоу: если `totp_enabled` — после верного пароля вернуть «нужен 2FA-код», принять код на отдельном шаге.
- [x] Rate limit на ввод кода (переиспользовать throttler).
- [ ] Обязательность 2FA для `platform_admin` — обсудить (мягко: предупреждение, не блок): в реализации 2FA опциональна для всех админ-ролей; решение владельца — отдельным шагом.
...
**Выполнено 2026-07-26 (§5.172).** Deviations: (1) вместо `otplib` — самописный `totp.util.ts` на `node:crypto` (RFC 6238, юнит-тесты на эталонных векторах приложения B; репо принципиально без крипто-зависимостей — как самописный HS256 в `crypto.util.ts`); для QR добавлена одна зависимость `qrcode` на бэке (data-URI, фронт без QR-библиотек); (2) имена эндпоинтов: `2fa/setup`+`2fa/confirm`+`2fa/disable`+`2fa/status` и `2fa/verify` (второй шаг логина) — план называл setup/verify/disable; (3) гейт стоит в `issueSessionForUser`, поэтому 2FA закрывает и magic-link, и ЕСИА (иначе обходилась бы), анти-replay шаг `totp_last_used_step`; (4) `INTEGRATION_CRYPTO_KEYS`/`ACTIVE_KEY_VERSION` задокументированы в прод-env (раньше отсутствовали, а прод без них не стартует).
...
- [x] Поднять контейнер Gotenberg (LibreOffice внутри) в dev и prod compose.
- [x] Добавить env `GOTENBERG_URL`, подключить (валидация схемы), healthcheck.
...
**Выполнено 2026-07-24 (§5.168, PR #308)** вместе с Task 3 одним PR. Deviations: healthcheck у `gotenberg` намеренно не задан (образ v8 может быть без `curl`; backend в Фазе 0 от него не зависит) — подключается в Фазе 1 вместе с рендером; дымовой curl из worker-сети отложен туда же.
...
- [x] Решить схему: полное шифрование поля + отдельный slepой хэш для точечного поиска/уникальности СНИЛС.
- [x] Мастер-ключ в env (для прода — свой; в dev — сгенерённый), запрет plaintext в БД.
- [x] Миграция данных существующих слушателей (их сейчас немного — один свой УЦ).
- [x] Обновить экспорт в реестры (`0045`/`0046`) — дешифровать на лету при выгрузке.
...
**Выполнено 2026-07-26 (§5.173).** Deviations по факту кода: (1) **текстовых полей паспорта в системе нет вовсе** (паспорт — только файлы-сканы identity-верификации с собственным retention) — шифруется единственное текстовое ПДн-поле `Learner.snils`; (2) learners хранятся **JSONB-документами** (`learning.mvp_runtime_documents` + stage1), а не колонками — поэтому вместо колонок миграция `0061` создаёт functional-индексы по слепому хэшу, а шифрование живёт на границе персистенса (`pii-crypto.ts` + `PostgresMvpPersistenceBackend`): при записи `snils` → AES-256-GCM + keyed-HMAC `snilsHash` (по нормализованным цифрам — маска и цифры дают один хэш), при чтении расшифровка; (3) благодаря этому **экспорты в реестры, ЕСИА-сверка и поиски не менялись вообще** — рантайм видит открытые значения; (4) миграция существующих данных — lazy: plaintext-строки читаются как есть и перешифровываются при первом сохранении состояния тенанта.
