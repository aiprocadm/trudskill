...
- [x] **Step 1: Написать падающие тесты (в `helpers.test.ts`, перед закрывающим `});` describe-блока, после строки 120)**
...
- [x] **Step 3: `model.ts` — добавить routeMeta для 2 сирот**
...
- [x] **Step 4: `model.ts` — русифицировать 3 метки в `navigationModel`**
...
- [x] **Step 5: `model.ts` — удалить 2 nav-записи заглушек**
...
- [x] **Step 6: `model.ts` — добавить 3 nav-записи сирот**
...
- [x] **Step 7: Запустить тесты — зелёные**

Run: `pnpm --filter @trudskill/frontend exec vitest run src/features/navigation/helpers.test.ts --no-file-parallelism`
Expected: PASS (в т.ч. инвариант §5.154 — все 3 новые ссылки резолвятся в routeMeta).

- [x] **Step 8: Прогнать весь фронтовый набор навигации — ничего не сломали**
...
- [x] **Step 1: Создать `nav-icons.ts` (курируемый реестр глифов)**
...
- [x] **Step 2: Проверить, что все имена глифов экспортируются установленной версией lucide-react**
...
- [x] **Step 3: Написать падающий тест `nav-groups.test.ts`**
...
- [x] **Step 4: Запустить — падает (нет модуля `nav-groups`)**
...
- [x] **Step 5: Создать `nav-groups.ts`**
...
- [x] **Step 1: Добавить `no-restricted-imports` во frontend-блок**
...
- [x] **Step 2: Проверить — правило ловит нарушение, `nav-icons.ts` чист**
...
- [x] **Step 3: Полный lint фронтенда (0 нарушений — прямых импортов lucide во фронте нет)**
...
- [x] **Step 1: Переписать `app-shell.tsx` (сайдбар-часть)**
...
- [x] **Step 2: Обновить `<style jsx>` — стили блоков + reduced-motion**
...
- [x] **Step 4: Существующие e2e (роутинг/видимость — по href) не сломаны**
...
- [x] **Step 5: Смоук-проверка в браузере (превью) — сайдбар и логин**
...
- [x] **Step 1: Падающий тест чистой логики `command-palette.test.ts`**
...
- [x] **Step 3: Создать `command-palette.ts`**
...
- [x] **Step 4: Запустить — зелёные**

Run: `pnpm --filter @trudskill/frontend exec vitest run src/features/navigation/command-palette.test.ts --no-file-parallelism`
Expected: PASS.

- [x] **Step 5: Создать компонент `widgets/shell/command-palette.tsx`**
...
- [x] **Step 6: Смонтировать палитру в `app-shell.tsx` + Ctrl/⌘+K + кнопка «Поиск»**
...
- [x] **Step 7: Smoke-тест `e2e/navigation-shell.e2e.test.ts`**
...
- [x] **Step 8: Typecheck + тесты + smoke**
...
- [x] **Step 9: Клавиатурная проверка в превью**
...
- [x] **Step 1: Обновить тесты `breadcrumbs.test.ts`**
...
- [x] **Step 3: Обновить `breadcrumbs.ts`**
...
- [x] **Step 4: Обновить рендер крошек в `app-shell.tsx`**
...
- [x] **Step 1: Лёгкая полировка топбара (без изменения поведения live-region)**
...
- [x] **Step 2: Полный фронтовый прогон**
...
- [x] **Step 4: Верификация a11y/поведения в превью (свести воедино)**
...
- [x] **Step 5: Документация — README §2 + handoff §5.163 + галочки плана**
