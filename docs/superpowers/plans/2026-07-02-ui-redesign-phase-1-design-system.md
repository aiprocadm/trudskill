...
- [ ] **Step 4: Подмешать `baseVars` в провайдер темы**
...
- [ ] **Step 6: Линт затронутых файлов и коммит**
...
- [ ] **Step 3: Составить полный список вхождений**
...
- [ ] **Step 4: Применить маппинг замен во всех файлах из списка**
...
- [ ] **Step 5: Прогнать тест-страж и весь пакет**
...
- [ ] **Step 1: Установить зависимость в оба пакета**
...
- [ ] **Step 4: Добавить CSS для флекс-раскладки и иконки**
...
describe('Skeleton — заглушка загрузки', () => {
  it('контейнер объявлен как status с русской меткой', () => {
    const el = Skeleton({});
    expect(el.props.role).toBe('status');
    expect(el.props['aria-label']).toBe('Загрузка');
  });

  it('kind=line рендерит N линий, каждая декоративная', () => {
    const el = Skeleton({ kind: 'line', lines: 3 });
  ...
  it('kind=block рендерит один блок', () => {
    const el = Skeleton({ kind: 'block' });
    const rows = el.props.children as ReactElement[];
    expect(rows).toHaveLength(1);
    expect(rows[0]?.props.className).toBe('ui-skeleton-block');
  });
});
```
...
// Скелетон поверх готовых CSS-классов ui-skeleton-line / ui-skeleton-block (shimmer уже в foundation).
// Контейнер — единый live-region, сами полосы декоративные.
export const Skeleton = ({
  ...
  <div role="status" aria-live="polite" aria-label="Загрузка">
    {Array.from({ length: kind === 'line' ? Math.max(1, lines) : 1 }, (_, index) => (
      <div
        key={index}
        className={kind === 'line' ? 'ui-skeleton-line' : 'ui-skeleton-block'}
        aria-hidden={true}
      />
    ))}
  </div>
...
- [ ] **Step 5: Прогнать тесты и коммит**
...
- [ ] **Step 4: Добавить CSS заголовка каллаута**
...
- [ ] **Step 4: Прогнать тесты пакета и типы фронтенда**
...
- [ ] **Step 4: Прогнать тесты ui и фронтенда**
...
- [ ] **Step 2: Обновить README §2 «AI Agent State»**
...
- [ ] **Step 3: Добавить запись в LMS_AGENT_HANDOFF §5**
...
- [ ] **Step 4: Отметить выполненные чекбоксы в этом плане и закоммитить документацию**
