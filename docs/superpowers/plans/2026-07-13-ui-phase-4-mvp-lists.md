...
**D1 — принятое микро-отклонение «устаревшие данные + ошибка» (Task 2 Step 2, распространяется на все 6 экранов).** Хуки списков построены на `useMvpQuery` → react-query (`apps/frontend/src/features/mvp/hooks.ts:42`), клиент с `staleTime: 30_000`, `refetchOnWindowFocus: false` (`src/lib/query/provider.tsx`). react-query удерживает `query.data` при ошибке рефетча **того же** ключа. Достижимо: кнопка «Обновить» у `CommissionsPage` или refetch-on-mount после 30с. В этом состоянии:

- **Было:** баннер `SectionError` **+ устаревший список/таблица** (тело рендерилось безусловно/при `data?.items.length`).
- **Стало:** `AsyncSection` показывает **только ошибку** (одно состояние за раз; ранний `return` на ошибке).

Отклонение **принято владельцем-паттерном**: это ровно поведение «одно состояние за раз» из смёрдженного пилота Фазы 3 (`learners-list` → `ListPage`→`AsyncSection`, PR #302). Смена фильтра создаёт НОВЫЙ query-ключ (нет устаревших данных) → там поведение идентично. Во всех обычных состояниях (первая загрузка / первая ошибка / успех / пусто) DOM побайтово прежний.

_Прочие проверки: чистка импортов не потребовалась — `LoadingState`/`SectionError`/`SectionEmpty` остаются в использовании у экранов-карточек/форм/дашбордов того же файла (42 вхождения)._
