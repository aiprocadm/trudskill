...
### Минимальный smoke по ролям (перед пилотом)

Используйте тестовые учётные записи из seed/документации окружения. Цель — убедиться, что сессия, маршруты и ответы API согласованы с ожидаемыми правами, без регрессий редиректов и envelope.

| Роль                    | Маршруты (примеры)                                                                                                | Что проверить                                                                                                                                                  |
| ----------------------- | ----------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Слушатель               | `/login` → `/learner/courses` или `/student/dashboard`, карточка курса `/learner/courses/[id]`                    | Вход, список назначенных курсов, открытие материала/прогресса; при наличии сценария — страница `/assessment`                                                   |
| Преподаватель / куратор | `/courses`, `/teacher/grading-center`, при необходимости `/groups`                                                | Доступ к курсам и проверке работ (без 403 на ожидаемых действиях)                                                                                              |
| Администратор           | `/users`, `/learners`, карточка `/learners/[id]`, `/admin/cockpit`, `/reports`, при необходимости `/integrations` | Пользователи/роли, реестр и карточка слушателя (`GET /learners/:id`), KPI на `/reports`, отсутствие утечки данных в UI при смене tenant (только свой контекст) |
| Общее для всех ролей    | `/notifications`, `/chat`, `/workspace`                                                                           | Загрузка списков, отсутствие необработанных 5xx                                                                                                                |

Канонические автоматические регрессы для приёмки — в [README](../README.md) (блок «Канонический E2E для приёмки ТЗ», quality gate `pnpm -s ci:check`).
...
- **Нагрузка:** после фиксации SLA в NFR — отдельный профиль на `POST /enrollments/bulk` и отчёты (см. TZ_TRUDSKILL_BASE.md §706–709).
