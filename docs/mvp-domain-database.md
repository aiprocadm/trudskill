...
Срез 1a (§5.560): после бэкфилла таблицы `crm.counterparties` и `learning.groups` **не отстают от снимка** — `PostgresMvpPersistenceBackend.projectChanged` на каждом сохранении пишет изменённые сущности (поштучные отпечатки `InMemoryMvpState.changedEntities`) той же транзакцией, только для авторитетной таблицы (РМ35); отказ строки — `learning.mvp_reconciliation_log` с `issue_type = projection_failed`, снимок сохраняется. Чтение из таблиц включается флагом `LMS_NORMALIZED_COLLECTIONS` (срез 1b). Срез 2a (§5.562): то же для `learning.learners` — ПДн только в `*_enc` и `snils_hash`, `user_id` только существующий из `iam.users`; стирание ПДн по 152-ФЗ обнуляет колонки при следующем сохранении. Срез 3a (§5.564): `learning.enrollments` и `learning.enrollment_status_history` — проекция после слушателей и групп, история каскадом при удалении; подстановки дат для NOT NULL помечены в `payload.__synthesized` и обратной проекцией не возвращаются. Срез 3b (§5.565): миграция 0111 — `learning.learners.linked_iam_user_id` (привязка «как в снимке», без FK) для anti-IDOR списка зачислений в SQL; после неё на стендах повторить бэкфилл.
