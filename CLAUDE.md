...
3. Code + tests; described in [LMS_AGENT_HANDOFF.md](LMS_AGENT_HANDOFF.md) §5.\* (sequentially numbered, currently up to §5.588 — always take the next number from the file) and [docs/TZ_MVP_TRACEABILITY.md](docs/TZ_MVP_TRACEABILITY.md) (BL → file paths).
...
- **Migrations** are numbered SQL in [`apps/backend/migrations/`](apps/backend/migrations/). **Don't edit historical files.** Latest is `0113_lookup_positions_education_countries.sql` as of 2026-09-24. Pick the next free number for new ones (numbers in the CDOPROF migration TZ are proposals).
