...
V1 roadmap (см. [docs/superpowers/plans/2026-05-21-cdoprof-v1-roadmap.md](docs/superpowers/plans/2026-05-21-cdoprof-v1-roadmap.md)) — **Phase 1 + Pillar A + Phase 2 (A+B+C) + Phase 3 (Plan A + Plan B + Plan C)** реализованы и слиты (Plan C — PR #215). **Wave 1** — **Plan 1 (модульный гейтинг + время)** слит (PR #218), **Plan 2 (аутентификация перед экзаменом №816)** слит (PR #219). **Wave 2 (регуляторные выгрузки)** — **под-цель B (реестр обученных по ОТ, Минтруд/ЕИСОТ, ПП №2464)** слита (PR #222; provisional-шаблоны + XML #223); **под-цель A (ФИС ФРДО, Рособрнадзор, по выданным документам об обучении)** реализована на ветке `feat/2026-06-03-frdo-registry-export` (durable MVP-модуль `frdo-registry/`, изоморфна ОТ). **V1.1 антивирус-гейт полностью закрыт и слит (PR #224)**. Стабильный quality gate `pnpm -s ci:check` (Cyrillic-path fallback на isolated backend runs — см. CLAUDE.md Gotchas); миграции до **0046**.

### Current Goal

Смерджить **Wave 2 под-цель A (ФИС ФРДО)** (ветка `feat/2026-06-03-frdo-registry-export`, зелёная; PR после ревью). Артефакты ФРДО — **PROVISIONAL** (2 swap-точки: сид `lookup.frdo_document_kinds` в migration 0046 + `COLUMNS` в `frdo-registry-xlsx.writer.ts`); подставить офиц. Excel-шаблон ФРДО + перечень видов документов ПО/ДПО перед боевой подачей. Затем под-цель **C (ЕИСОТ «лица на тестирование»)** + 3 офиц. артефакта ЛКОТ для ОТ-реестра; параллельно — Phase 4 (ЕСИА/прокторинг). См. [дорожную карту паритета](docs/superpowers/specs/2026-05-30-legacy-parity-roadmap.md).

### Last Completed Task

**Wave 2 — Plan A: выгрузка в ФИС ФРДО (Рособрнадзор)** (2026-06-03, ветка `feat/2026-06-03-frdo-registry-export` off `main` #224; brainstorming→plan→executing-plans+TDD по [плану](docs/superpowers/plans/2026-06-03-frdo-registry-export.md)). Выгрузка `.xlsx` **по выданным документам об обучении** (удостоверения ПК / дипломы ПП) для ручной загрузки в ЛК ФРДО. **Backend** (durable MVP-модуль `frdo-registry/`, НЕ пустой `FrdoAdapter`): migration `0046` (provisional-классификатор `lookup.frdo_document_kinds` PK/PP + `learners.date_of_birth`); источник — `DocumentsService.listIssuedDocuments` (одна строка = один документ, без раунд-трипа, только `.xlsx`); `buildFrdoRows` + `validateFrdoRow` (жёсткие поля + опц. СНИЛС) + `FrdoRegistryXlsxWriter` (`COLUMNS` swap-2) + `FrdoRegistryService` (джойн документ→слушатель/курс, искл. archived/revoked, persist batch+records, audit `regulatory.frdo_exported`); 4 эндпоинта `/frdo-registry/*` под `regulatory.export.read/write`. `Learner.dateOfBirth` через create/update/DTO/bulk-import. **Frontend**: секция «ФИС ФРДО» на `/gov-export` + `Frdo*` api/hooks/types/contract. Тесты: backend кластер **131** (6 файлов) + frontend contract 4 + e2e 4; `tsc` **8/8**, ESLint clean. **PROVISIONAL** — 2 swap-точки + баннер; follow-up — фронт Excel-колонка «Дата рождения». Отклонения — план §«Known deviations». Handoff §5.103. **Предыдущее:** V1.1 AV-гейт Задачи 7-13 (#224 merged, §5.102); ОТ-реестр provisional+XML (#223, §5.101); Wave 2 Plan B (#222, §5.100).

### Current Task

**Wave 2 под-цель A (ФИС ФРДО) готова** на ветке `feat/2026-06-03-frdo-registry-export` (зелёная: backend кластер 131 + frontend contract 4 + e2e 4, `tsc` 8/8) — PR после ревью. Артефакты **PROVISIONAL** (сид `lookup.frdo_document_kinds` + `COLUMNS` в `frdo-registry-xlsx.writer.ts`), сверить с офиц. Excel-шаблоном ФРДО + перечнем видов документов ПО/ДПО перед боевой отправкой. V1.1 AV-гейт слит (#224); ОТ-реестр provisional + XML слиты (#223).

### Next Task

**Wave 2 продолжение:** под-цель **C (ЕИСОТ «лица на тестирование»)** — отдельный план (brainstorming→spec→plan). Подставить офиц. артефакты регистров: ФРДО (Excel-шаблон → `COLUMNS` в `frdo-registry-xlsx.writer.ts`; виды документов ПО/ДПО → сид `lookup.frdo_document_kinds`, migration 0046) + 3 артефакта ЛКОТ для ОТ-реестра (сид `0045`, `COLUMNS`/`RESPONSE_COLUMNS` в `ot-registry-*`). Параллельно — Phase 4 (ЕСИА / прокторинг). Малые follow-up: фронт Excel-колонка «Дата рождения» в bulk-импорте (бэкенд готов); реальный email-адаптер №816; tenant-default pre-exam «вкл. для новых групп». См. [дорожную карту паритета](docs/superpowers/specs/2026-05-30-legacy-parity-roadmap.md) §3–§4.
...
AI Agent (Wave 2 Plan A — ФИС ФРДО: durable MVP-модуль `frdo-registry/`, выгрузка `.xlsx` по выданным документам, migration 0046 + `Learner.dateOfBirth`, секция на `/gov-export`; brainstorming→plan→executing-plans+TDD; backend кластер 131 + frontend 8 зелёные, tsc 8/8, ESLint clean; PROVISIONAL — 2 swap-точки)

### Last Updated At

2026-06-03 (Wave 2 Plan A — ФИС ФРДО на ветке `feat/2026-06-03-frdo-registry-export`; previous: V1.1 AV #224, ОТ-реестр provisional #223, Wave 2 Plan B #222, Wave 1 #218/#219, Phase 3 Plan C #215, Phase 2 A/B/C, Phase 1 §4.3 + Pillar A)
