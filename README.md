...
**Phase 4 Plan A — идентификация личности** готова на ветке `feat/2026-06-10-phase-4-plan-a-identity-verification` (§5.117; spec + plan в `docs/superpowers/`; 13 задач subagent-driven + two-stage review + финальное холистическое ревью с фиксами C1/I1). Изолированные прогоны: backend-кластер 214/214 (+ runner/scanner/lifecycle), frontend identity 23 + e2e 13, миграции 42/42, `pnpm typecheck` 8/8. **PR #240** (включая merge `origin/main` после #238/#239).

### Next Task

**Merge PR #240 (Phase 4 Plan A)**. Затем по выбору владельца: **Phase 4 Plan B (прокторинг)** / **Phase 9 (SCORM+аналитика)** / **Phase 10 (PWA/WCAG/Excel)** — все без внешних блокеров; параллельно **deploy-execution** (владелец: сервер + DNS + SMTP, прогон `infra/server-setup.md` + `infra/bootstrap-admin.md`). Ops-активация retention-cron: `IDENTITY_IMAGE_RETENTION_ENABLED=true` после подтверждения 90-дневной политики. **Отложено по решению владельца:** ЕСИА (подключится к Phase 4 позже), НЭП (Phase 6), Pruffme/ЮKassa (Phase 7/8), clamav, управляемые сервисы — в свои фазы.
