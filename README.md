...
**2026-06-13 (эта сессия):** **Phase 10 Track C — PWA + web-push** реализована на ветке `feat/2026-06-13-phase-10c-pwa-push` (§5.121, план `docs/superpowers/plans/2026-06-13-phase-10c-pwa-push.md`, 15 задач, параллельно с Track B Excel-конструктор). **Всё DORMANT, безопасно к мёрджу** (`WEB_PUSH_ENABLED=false` по умолчанию → `NoopWebPushSender` → email-поведение байт-в-байт прежнее). Backend: env `WEB_PUSH_ENABLED`+`VAPID_*` (conditional-required); MVP-коллекция `pushSubscriptions` (**без миграции**, зарегистрирована в обоих местах); `PushSubscriptionService` (request-scoped CRUD, tenant/user-изоляция, audit) + чистый `push-subscription-store`; **singleton `WebPushSender`** (через reentrant `MvpTenantRunner`, web-push lib, 404/410-cleanup) + `NoopWebPushSender` по `WEB_PUSH_ENABLED`; **канал push в `NotificationDispatcher`** (фан-аут после email-цикла по `recipient.userId` из `linkedIamUserId`; 5 call-sites Phase 5 не тронуты структурно); self-service эндпоинты `web-push/*` (**TenantGuard-only, без RBAC/миграции**). Frontend: PWA-манифест (`app/manifest.ts`) + плейсхолдер-иконки; **Serwist** SW (`@serwist/next`, `src/app/sw.ts` app-shell + push/notificationclick-хендлеры, SCORM-rewrites сохранены); `features/push/` (чистая VAPID-логика + api + `usePushSubscription` + `PushSettingsScreen` встроен в `/notifications`). Изолированные прогоны: backend таргет **73**, frontend полный **566/566**, typecheck **8/8**, ESLint clean; frontend build генерит `sw.js`+`manifest.webmanifest` (standalone-trace падает Windows-EPERM — environmental, CI чист). PR не создан — за оркестратором. Ручная SW/push end-to-end проверка отложена (нет DOM).

**Предыдущее (2026-06-11, вечер):** **Phase 4 Plan B — прокторинг (запись видео итогового теста)** реализована на ветке `feat/2026-06-11-phase-4-plan-b-proctoring` (§5.118), **Phase 4 закрыта целиком** (кроме отложенных владельцем ЕСИА/live-оператора): migration **0051** (`learning.proctoring_recordings` + `requires_proctoring` + per-student `proctoring_override` + права `proctoring.submit/read`), браузерный MediaRecorder → 30-сек webm-чанки → presigned PUT (лимит 10MB files-слоя), **5-й гейт `assertProctoringGate`** в `startAttempt` (412 `proctoring_required`, non-collision с Wave 1/Plan A), согласие 152-ФЗ + превью камеры, `● REC` + stop-on-submit + **resume-баннер после F5**, админ-очередь `/admin/proctoring-recordings` с плеером-склейкой чанков (AV-гейт, presigned GET, предупреждения о разрывах), override-select на странице группы, dormant **365-дневный** retention-cron (`PROCTORING_VIDEO_RETENTION_ENABLED=false`, lock 528_493, write-режим runner + регресс-guard). Финальное холистическое ревью снова оправдалось: **CRITICAL C1 — ретрай чанка запрашивал новый intent → 409 → мёртвый ретрай + фантомный чанк ронял всю сборку видео** (фикс: intent-кэш per-sequence + per-chunk деградация плеера) + I1 (F5 убивал запись навсегда → resume-баннер) + I2 (override был API-only → UI). Миграции теперь до **0051**. PR не создан на момент записи — см. Current Task.
