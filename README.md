...
**2026-06-10 (эта сессия):** Обе ветки 2026-06-08 **слиты в `main`**: Phase 0 деплой-фундамент (**#235**) + Production auth readiness A+B (**#237**; плюс **#236** — boot-fix под tsx). Закрыт follow-up §3.3 плана auth-readiness: `infra/bootstrap-admin.md` приведён к новой реальности (email-доставка magic-link по `NOTIFICATIONS_EMAIL_ENABLED`, авто-`SeedCredentialHygiene` при прод-буте, ручные psql-шаги → подстраховка) → ветка `docs/2026-06-10-bootstrap-admin-auth-readiness`, §5.115. **Кодовых блокеров пилота не осталось** — следующий шаг deploy-execution владельцем.
