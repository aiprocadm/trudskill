...
V1 roadmap (см. [docs/superpowers/plans/2026-05-21-cdoprof-v1-roadmap.md](docs/superpowers/plans/2026-05-21-cdoprof-v1-roadmap.md)) — **Phase 1 + Pillar A + Phase 2 (A+B+C) + Phase 3 (Plan A + Plan B + Plan C)** реализованы и слиты (Plan C — PR #215). **Wave 1** — **Plan 1 (модульный гейтинг + время)** слит (PR #218), **Plan 2 (аутентификация перед экзаменом №816)** слит (PR #219). **Wave 2 (регуляторные выгрузки)** — **под-цель B (реестр обученных по ОТ, Минтруд/ЕИСОТ, ПП №2464)** слита (PR #222; provisional-шаблоны + XML #223); **под-цель A (ФИС ФРДО, Рособрнадзор, по выданным документам об обучении)** слита (**PR #225**, durable MVP-модуль `frdo-registry/`); **под-цель C (ЕИСОТ «лица на тестирование», Минтруд/ЛКОТ)** реализована на ветке `feat/2026-06-03-eisot-testing-registry-export` (durable MVP-модуль `eisot-testing-registry/`, ростер по фильтру через зачисления, **без миграции**, изоморфна ФРДО). **V1.1 антивирус-гейт полностью закрыт и слит (PR #224)**. Стабильный quality gate `pnpm -s ci:check` (Cyrillic-path fallback на isolated backend runs — см. CLAUDE.md Gotchas); миграции до **0046**.

### Current Goal

Смерджить **Wave 2 под-цель C (ЕИСОТ «лица на тестирование»)** (ветка `feat/2026-06-03-eisot-testing-registry-export`, зелёная; PR после ревью). Артефакт ЕИСОТ — **PROVISIONAL** (1 swap-точка: `COLUMNS` в `eisot-testing-xlsx.writer.ts`); подставить офиц. Excel-шаблон ростера ЛКОТ перед боевой подачей. Затем подставить офиц. артефакты регистров ФРДО (#225: сид `lookup.frdo_document_kinds` + `COLUMNS`) и ОТ-реестра (сид `0045`, `COLUMNS`/`RESPONSE_COLUMNS`); параллельно — Phase 4 (ЕСИА/прокторинг). См. [дорожную карту паритета](docs/superpowers/specs/2026-05-30-legacy-parity-roadmap.md).

### Last Completed Task

**Wave 2 — Plan C: выгрузка в ЕИСОТ «лица на тестирование» (Минтруд / ЛКОТ)** (2026-06-03, ветка `feat/2026-06-03-eisot-testing-registry-export` off `origin/main` #225; brainstorming→spec→plan→executing-plans+TDD по [плану](docs/superpowers/plans/2026-06-03-eisot-testing-registry-export.md)). Выгрузка `.xlsx` — **ростер «кого предстоит протестировать»** по фильтру (группа/период/клиент) для ручной загрузки в ЛКОТ. **Backend** (durable MVP-модуль `eisot-testing-registry/`, НЕ пустой `EisotAdapter`; **без миграции** — все поля уже в `main`): источник — `MvpService.listEnrollments` (**одна строка = один слушатель**, дедуп, без экзамена/документа, без раунд-трипа); `buildEisotTestingRows` + `validateEisotTestingRow` (жёсткие ФИО+работодатель; опц. СНИЛС/ИНН) + `EisotTestingXlsxWriter` (`COLUMNS` swap-1) + `EisotTestingRegistryService` (Scope.REQUEST: фильтр зачислений + ре-фильтр `enrolledAt`, искл. `cancelled`, джойн learner/counterparty/course, дедуп, persist batch+records, audit `regulatory.eisot_testing_exported`); 4 эндпоинта `/eisot-testing-registry/*` под `regulatory.export.read/write` (переиспользованы из 0045). **Frontend**: секция «ЕИСОТ — лица на тестирование» на `/gov-export` + `EisotTesting*` api/hooks/types/contract. Тесты: backend кластер **41** (5 файлов) + dto-validation 3 + frontend contract 5 + e2e 4; `tsc` **8/8**, ESLint clean. **PROVISIONAL** — 1 swap-точка + баннер. Отклонения — план §«Known deviations». Handoff §5.104. **Предыдущее:** Wave 2 Plan A — ФИС ФРДО (#225 merged, §5.103); V1.1 AV-гейт Задачи 7-13 (#224, §5.102); ОТ-реестр provisional+XML (#223, §5.101).

### Current Task

**Wave 2 под-цель C (ЕИСОТ «лица на тестирование») готова** на ветке `feat/2026-06-03-eisot-testing-registry-export` (зелёная: backend кластер 41 + dto-validation 3 + frontend contract 5 + e2e 4, `tsc` 8/8, ESLint clean) — PR после ревью. Артефакт **PROVISIONAL** (`COLUMNS` в `eisot-testing-xlsx.writer.ts`), сверить с офиц. Excel-шаблоном ростера ЛКОТ перед боевой отправкой. Под-цель A (ФИС ФРДО) слита (#225); V1.1 AV-гейт слит (#224).

### Next Task

**Wave 2 — все три выгрузки (A ФРДО / B ОТ-реестр / C ЕИСОТ) реализованы.** Главный остаток — **подставить офиц. артефакты регистров** перед боевой подачей: ЕИСОТ (Excel-шаблон ростера ЛКОТ → `COLUMNS` в `eisot-testing-xlsx.writer.ts`), ФРДО (Excel-шаблон → `COLUMNS`; виды документов ПО/ДПО → сид `lookup.frdo_document_kinds`, migration 0046), ОТ-реестр (3 артефакта ЛКОТ → сид `0045` + `COLUMNS`/`RESPONSE_COLUMNS` в `ot-registry-*`). Параллельно — Phase 4 (ЕСИА / прокторинг). Малые follow-up: фронт Excel-колонка «Дата рождения» в bulk-импорте (бэкенд готов); реальный email-адаптер №816; tenant-default pre-exam «вкл. для новых групп». См. [дорожную карту паритета](docs/superpowers/specs/2026-05-30-legacy-parity-roadmap.md) §3–§4.
...
AI Agent (Wave 2 Plan C — ЕИСОТ «лица на тестирование»: durable MVP-модуль `eisot-testing-registry/`, ростер `.xlsx` по фильтру через зачисления, **без миграции**, секция на `/gov-export`; brainstorming→spec→plan→executing-plans+TDD; backend кластер 41 + dto 3 + frontend 9 зелёные, tsc 8/8, ESLint clean; PROVISIONAL — 1 swap-точка)

### Last Updated At

2026-06-03 (Wave 2 Plan C — ЕИСОТ «лица на тестирование» на ветке `feat/2026-06-03-eisot-testing-registry-export`; previous: Wave 2 Plan A — ФИС ФРДО #225, V1.1 AV #224, ОТ-реестр provisional #223, Wave 2 Plan B #222, Wave 1 #218/#219, Phase 3 Plan C #215, Phase 2 A/B/C, Phase 1 §4.3 + Pillar A)
