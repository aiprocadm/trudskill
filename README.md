...
V1 roadmap (см. [docs/superpowers/plans/2026-05-21-cdoprof-v1-roadmap.md](docs/superpowers/plans/2026-05-21-cdoprof-v1-roadmap.md)) — **Phase 1 + Pillar A + Phase 2 (A+B+C) + Phase 3 (Plan A + Plan B + Plan C)** реализованы и слиты (Plan C — PR #215). **Wave 1** — **Plan 1 (модульный гейтинг + время)** слит (PR #218), **Plan 2 (аутентификация перед экзаменом №816)** слит (PR #219). **Wave 2 (регуляторные выгрузки)** — **под-цель B (реестр обученных по ОТ, Минтруд/ЕИСОТ, ПП №2464)** слита (PR #222). **V1.1 антивирус-гейт полностью закрыт**: ядро (#216-217) + **Задачи 7-13** (проактивный скан при submit, `antivirusStatus` в чтении сабмишена/очереди ревью, HTTP-гейт boundary, UI ревьюера/слушателя) на ветке `feat/2026-06-02-v1.1-antivirus-tasks-7-13`. Стабильный quality gate `pnpm -s ci:check` (Cyrillic-path fallback на isolated backend runs — см. CLAUDE.md Gotchas); миграции до **0045**.

### Current Goal

Смерджить **V1.1 AV-гейт Задачи 7-13** (ветка `feat/2026-06-02-v1.1-antivirus-tasks-7-13`, зелёная) — gate полностью закрыт в коде, остаётся ops-задача (поднять clamd + `ANTIVIRUS_ENABLED=true`). Затем продолжить **Wave 2**: подставить **3 официальных артефакта ЛКОТ** (классификатор программ, колонки `.xlsx`, формат файла-ответа — изолированы в 3 местах) и под-цели **A (ФИС ФРДО)** / **C (ЕИСОТ «лица на тестирование»)** — см. [дорожную карту паритета](docs/superpowers/specs/2026-05-30-legacy-parity-roadmap.md). OT-registry provisional-шаблоны + XML слиты (#223).

### Last Completed Task

**V1.1 — антивирус-скан-гейт: Задачи 7-13** (2026-06-02, ветка `feat/2026-06-02-v1.1-antivirus-tasks-7-13` off `origin/main`; executing-plans + TDD по [плану](docs/superpowers/plans/2026-05-30-v1.1-antivirus-scan-gate.md)). Завершает gate, ядро которого (Задачи 1-6) уже в проде (#216-217). **Backend:** проактивный best-effort скан в `submitAssignmentSubmission` (fire-and-forget + `avScanLogger`; gate лениво пересканит при скачивании); `FilesService.getAntivirusStatuses` (батч, без N+1) + `getAntivirusStatus`; `getAssignmentSubmission` и `getReviewerQueue` стали `async` и отдают `antivirusStatus` (для очереди `fileId` проброшен через pure-aggregator `aggregateReviewerQueue`, статус подмешан в сервисе); HTTP-гейт boundary в `plan-c.http.integration.test.ts` через реальный `FilesService` (clean→200, infected→423 `file_infected`, error→409 `file_scan_failed`). **Frontend:** тип `AntivirusStatus` + `antivirusStatus` в DTO сабмишена и `ReviewerQueueItem`; `formatAntivirusStatus` (ревьюер) гейтит кнопку «Скачать файл» (только `clean`) + показывает ошибку гейта; `formatAntivirusStatusLearner` + статус на экране сдачи через существующий `useSubmission`. Tests: добавлены backend (scan 5, getAntivirusStatus(es) 4, gate-boundary 3) — таргет-набор зелёный (63 в кластере + mvp.service 93/93), frontend +12 (format 3+4, contract +2 + queue); `tsc` 8/8, ESLint clean. **Отклонения от плана:** (1) reviewer-queue — обогащение в сервисе, т.к. очередь строит pure-функция (не inline-массив); (2) экран слушателя ведёт `LearnerAssignmentSummary` (без fileId) → статус берётся через существующий `useSubmission` (полный DTO), не из summary; (3) добавлен `avScanLogger` вместо тихого глотания ошибки. Единственный остаток — ops (поднять clamd, `ANTIVIRUS_ENABLED=true`). Handoff §5.102. **Предыдущее:** Wave 2 Plan B — выгрузка в реестр обученных по ОТ (#222 merged, §5.100); OT-registry provisional templates + XML (#223 merged, §5.101); PLANS_STATUS-консолидация (§5.99, #220).

### Current Task

**V1.1 AV-гейт Задачи 7-13 готовы** на ветке `feat/2026-06-02-v1.1-antivirus-tasks-7-13` — в **PR #224** (ждёт review/merge; затем ops: поднять clamd + `ANTIVIRUS_ENABLED=true`). ОТ-реестр provisional-шаблоны + XML XSD 1.0.3 + выбор формата xlsx|xml **слиты (#223)** — артефакты PROVISIONAL, сверить с эталоном ЛКОТ перед боевой отправкой. Дальше — продолжение Wave 2: 3 официальных артефакта ЛКОТ + под-цели A (ФРДО) / C (ЕИСОТ-тестирование).
...
AI Agent (V1.1 AV-гейт Задачи 7-13: проактивный скан при submit + `antivirusStatus` в DTO чтения/очереди + HTTP-гейт boundary + UI ревьюера/слушателя; executing-plans + TDD; backend таргет-набор зелёный + frontend format/contract; tsc 8/8, ESLint clean; остаток — ops clamd. Перед PR влит `origin/main` #223 — ОТ-реестр provisional)

### Last Updated At

2026-06-02 (V1.1 AV-гейт Задачи 7-13 в PR #224; previous: ОТ-реестр provisional #223, Wave 2 Plan B #222, Wave 1 Plan 2 №816 #219 / Plan 1 #218, V1.1 AV ядро #217, Phase 3 Plan C #215, Phase 2 A/B/C, Phase 1 §4.3 + Pillar A)
