...
describe('меню человека вместо кнопки «Выйти» (ТЗ 7.1)', () => {
  it('состав меню — ровно тот, что назвали ТЗ', () => {
    /*
     * Состав ЗАКРЫТ: каждый пункт назван требованием, лишнего быть не должно. Меню человека —
     * последнее место, куда стоит складывать «ещё одну полезную ссылку»: оно есть на каждом
     * экране, и разрастание превращает его в свалку.
     *
     * Четвёртый пункт добавлен осознанно: ТЗ 15.5 требует «Сообщить о проблеме» именно в меню
     * профиля (журнал 583). Порядок тоже значим — выход остаётся последним.
     */
    expect(
      userMenuItems(sessionOf(['tenant_admin'], 'Иванов Пётр')).map((item) => item.label)
    ).toEqual(['Профиль', 'Оформление', 'Сообщить о проблеме', 'Выйти']);
  });
