 ...
 *
 * **Уточнение признака (ТЗ 8.3).** Панель руководителя — три блока, в одном из которых
 * небольшая таблица по компаниям. По прежнему признаку она считалась «реестром», и сторож
 * требовал одеть её в каркас списка с фильтрами, выбором колонок и постраничностью. Это
 * означало бы превратить обзорный экран в реестр — ровно то, чего ТЗ не просило.
 *
 * Поэтому дашборд отделён от реестра по составу: реестр — ОДИН предмет под шапкой, дашборд
 * складывается из нескольких блоков (`SectionCard`). Инвариант «новый реестр собирается на
 * общем каркасе» остаётся в силе; сузилось только то, что считать реестром.
 */
const isHandMadeRegistry = (source: string): boolean => {
  ...
  if ((source.match(/<SectionCard/g) ?? []).length >= 2) return false;
