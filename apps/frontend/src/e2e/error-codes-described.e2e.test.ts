...
describe('TXT-004 · про каждый код ошибки принято решение', () => {
  ...
  /*
   * Ревизия 2026-09-07 (§5.426): инвентарь не видел броски через свои классы поверх исключений
   * Nest, и шесть кодов не проходили эту сверку вовсе. Молчание сторожа читалось как «решение
   * принято» — а решения не было. Проверка прямая, чтобы слепота не вернулась незаметно.
   */
  it('инвентарь видит коды, которые бэкенд отдаёт через свои классы', () => {
    for (const code of [
      'tenant_state_conflict',
      'duplicate_document_number',
      'order_not_payable'
    ]) {
      expect(inventory.codes.has(code), `${code} не найден инвентарём`).toBe(true);
    }
  });
});
