...
export const TOP_JOB_ROUTES: Record<string, TopJobRoute> = {
  ...
  /*
   * ТЗ 8.3: обе задачи решает сама панель — руководитель открывает её и видит ответ, без
   * выбора строки и без нажатий. «Выдать документы группе» отсюда убрана: права
   * `documents.generate` у руководителя нет, и путь вёл его к кнопке, которая ответит
   * отказом (журнал 530).
   */
