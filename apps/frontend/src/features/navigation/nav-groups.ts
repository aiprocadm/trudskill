...
export const NAV_GROUPS: NavGroup[] = [
  {
    ...
    // `/tasks` — задачи сотрудников (ТЗ перехода с CDOPROF §II.3): обзорный блок, как и панели.
    hrefs: ['/workspace', '/manager', '/methodist', '/learning/calendar', '/tasks']
  },
  {
    id: 'my-learning',
    label: 'Моё обучение',
  ...
  },
  {
    ...
    hrefs: [
      ...
      '/admin/operations',
      /*
       * ТЗ 12.2: «Фоновые задачи» — что стало с тем, что человек отправил. Рядом с
       * «Эксплуатацией» намеренно: та показывает ЗАСТРЯВШЕЕ и адресована администрации,
       * эта — свои долгие операции и адресована тому, кто их поставил.
       */
 ...
 *
 * Отделена от getGroupedNavigation, потому что нижняя часть меню (разделы вне короткого
 * списка роли)
...
/**
 * Схлопывание групп из одного пункта (ТЗ «Стабилизация, UX и развитие», 3.2 / Н4).
 *
 * **Зачем.** Меню роли собиралось вычитанием из общего, и от блока нередко оставался ОДИН
 * пункт. Живая проверка по снимку прав из базы: у методиста блок «Отчёты и выгрузки» состоял
 * из одних «Госвыгрузок», у администратора центра «Люди и группы» — из одной «Массовой
 * загрузки»; всего девять таких блоков у шести ролей из семи. Заголовок группы над
 * единственным пунктом — это лишний клик и обещание, что внутри есть что-то ещё.
 *
 * **Критерий приёмки ТЗ:** ни у одной роли нет группы с одним пунктом.
 *
 * Возвращает две части: одиночные пункты поднимаются на верхний уровень (`loose`), группы из
 * двух и более остаются группами. Порядок сохраняется — он задан `NAV_GROUPS`.
 */
export const collapseSingleItemGroups = (
  groups: NavGroupView[]
): { loose: NavigationItem[]; groups: NavGroupView[] } => ({
  loose: groups.filter((group) => group.items.length === 1).flatMap((group) => group.items),
