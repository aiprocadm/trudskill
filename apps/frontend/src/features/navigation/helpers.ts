 ...
 */
/**
 * Меню роли — ровно её чертёж, без дополнения до семи и без второго этажа «Ещё».
 *
 * Так помечены кабинеты, которые ТЗ описывает поимённо: слушатель (6.1, ровно пять пунктов) и
 * администратор платформы (8.1, никакой работы внутри центра). У человека с несколькими ролями
 * правило действует, только если ВСЕ его роли такие: иначе он потерял бы разделы второй роли.
 */
const hasExactNav = (session: UserSession | null): boolean => {
  const blueprints = getSessionRoleBlueprints(session);
  return blueprints.length > 0 && blueprints.every((item) => item.exactNav === true);
};
...
export const getNavigationView = (session: UserSession | null) => {
  ...
  /*
   * Кабинеты, описанные ТЗ поимённо, показываются ровно чертежом.
   *
   * Слушатель (6.1): пять пунктов, все на виду; ушедшие разделы открываются из содержимого —
   * «Задания», «Вебинары» и «Календарь» с главной кабинета, «Подтверждение личности» и
   * «Оплаты» из профиля (журнал 490, 491).
   *
   * Администратор платформы (8.1): работы внутри центра в его меню быть не должно; внутрь он
   * попадает через «Войти от имени» (журнал 527).
   */
  if (hasExactNav(session)) {
