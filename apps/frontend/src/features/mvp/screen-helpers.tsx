...
/**
 * Дата И ВРЕМЯ по-русски — канон для всего приложения (ТЗ 5.12.1).
 *
 * Зачем отдельно от `formatDate`: там, где события идут подряд внутри одного дня (журнал
 * действий, очередь задач выпуска, карантин), одна дата не даёт прочитать порядок — а
 * именно порядок и разбирают, когда выясняют, что за чем произошло.
 *
 * Почему здесь: одно и то же время печаталось ДВУМЯ разными способами — «18.09.2026, 11:45:12»
 * в эксплуатации и «2026-09-18 11:45» в проверке работ (журнал 479). Один и тот же момент,
 * записанный по-разному на соседних экранах, человек читает как разные данные.
 */
