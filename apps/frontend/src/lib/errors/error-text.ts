...
const BY_CODE: Record<string, ErrorText> = {
  ...
  // Ревизия 2026-08-27 (порция 25): прежний текст обещал, что сохранённые ответы «уже
  // отправлены», — на деле по просроченной попытке баллы не выставляются вовсе. Обещать
  // человеку то, чего система не делает, хуже, чем сказать прямо.
  attempt_expired: {
    what: 'Время на попытку истекло — она закрыта без оценки.',
    next: 'Баллы по ней не выставляются, даже если вы успели ответить. За новой попыткой обратитесь в учебный центр.'
  },
  attempt_terminal: {
