...
export const AppShell = ({ children }: PropsWithChildren) => {
  ...
  */
  ...
  /*
   * ТЗ 13.5: работа «от имени» видна ВСЕГДА, а не на одном экране. Полоса живёт в оболочке —
   * значит она над любой страницей кабинета, включая те, что появятся потом. Имя центра берётся
   * с сервера вместе с признаком режима; без него полоса всё равно показывается, но центр
   * называется словом, а не кодом — сырой идентификатор человеку запрещён (журнал 551).
   */
  const impersonation = impersonationBanner(session, session?.user.tenantName);
  ...
  return (
    /*
     * ТЗ 13.5: пометку получает КАЖДОЕ подтверждение опасного действия, а зовут его из десятка
     * экранов. Признак кладётся здесь, один раз: иначе первый же новый экран забыл бы про него
     * молча — подтверждение выглядело бы обычным, а действие ушло бы от чужого имени.
     */
    <ImpersonationProvider {...(impersonation ? { note: IMPERSONATION_CONFIRM_NOTE } : {})}>
      <div className={`app-shell ${collapsed ? 'app-shell--narrow' : ''}`}>
        <a href="#app-shell-main" className="app-shell__skip-link">
          Перейти к основному содержимому
        </a>
        {impersonation ? (
          <div className="app-shell__impersonation" role="status">
            <span>{impersonation.text}</span>
            <button
              type="button"
              className="ui-button-secondary"
              onClick={() => {
                /*
                 * Выход из режима — это выход из сессии: платформенная сессия завершилась в
                 * момент входа «от имени» (так сказано в самом подтверждении), и возвращаться
                 * некуда, кроме входа. Отдельной «обратной» ручки нет намеренно: она означала бы
                 * восстановление завершённой сессии.
                 */
                void logout().catch(() => setLogoutWarning('Не удалось выйти из режима'));
              }}
            >
