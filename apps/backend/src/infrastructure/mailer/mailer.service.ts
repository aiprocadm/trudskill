...
export interface EmailMessage {
  ...
  body: string;
  /**
   * Оформленная часть письма (ТЗ 11.2 пункт 2): логотип и цвет учебного центра.
   *
   * Необязательная намеренно. Текстовая часть уходит ВСЕГДА и остаётся главной: часть
   * почтовых программ и программы чтения с экрана показывают именно её, а картинки у
   * большинства получателей заблокированы по умолчанию. Оформление — добавка, а не замена.
   */
  html?: string;
