export interface IntegrationAdapter {
  ...
  prepareExportPayload(input: {
    exportType: string;
    sourceFilter: Record<string, unknown>;
  }): Promise<Record<string, unknown>>;
  sendExportBatch(input: {
    payload: Record<string, unknown>;
  }): Promise<{ status: 'completed' | 'partial_success'; externalBatchId: string }>;
  handleWebhook(input: {
    eventType: string;
    payload: Record<string, unknown>;
  }): Promise<{ status: 'accepted' | 'processed'; externalId?: string }>;
