...
describe('режим без базы ведёт себя так же', () => {
  ...
  /*
   * Ревизия 2026-09-07 (§5.428). Сортировка «по времени, новые сверху» — не полный порядок:
   * `now()` в Postgres это время НАЧАЛА ТРАНЗАКЦИИ, и все записи, сделанные одной транзакцией,
   * получают ОДИН И ТОТ ЖЕ момент. Массовое зачисление на двести человек кладёт в журнал
   * двести строк с одинаковым временем — и при листании база вправе вернуть их в разном
   * порядке на разных страницах: одна запись покажется дважды, другая не покажется ни разу.
   *
   * Журнал аудита — то, по чему разбирают спор. Расползающийся список здесь дороже всего.
   */
  it('порядок листания полный: время, а следом — неповторимое поле', async () => {
    const { service, queries } = makeService(10);
    await service.listPage('tenant_a', { limit: 5, offset: 5 });
