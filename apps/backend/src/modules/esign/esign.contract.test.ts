...
describe('Esign API contract', () => {
  ...
  beforeAll(async () => {
    ...
    @Controller('esign')
    class TestEsignController {
      @Get('applications')
      applications() {
        return {
          items: [{ id: 'app_1', learnerId: 'u_1', status: 'draft' }],
      ...
      processes() {
        return {
          items: [{ id: 'proc_1', generatedDocumentId: 'gdoc_1', status: 'in_signing' }],
          meta: { page: 1, pageSize: 20, total: 1 }
        };
      }
      @Get('legal-log')
      legalLog() {
        return {
          items: [{ id: 'log_1', eventType: 'esign.application.created', entityId: 'app_1' }],
          meta: { page: 1, pageSize: 20, total: 1 }
        };
      }
    }
    ...
    @Module({
      ...
      controllers: [TestEsignController]
    })
    class TestAppModule {}
  ...
  it('matches contract for GET /esign/applications', async () => {
    const response = await fetch(`${apiBaseUrl}/esign/applications`, {
