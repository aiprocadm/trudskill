...
export class PostMessageDto {
  @IsString()
  /*
   * ТЗ 16.3 (пункт 3 чек-листа приёмки): имени поля в тексте для человека быть не должно.
   * Здесь стояло «textBody: сообщение не может быть пустым» — читатель видел внутреннее имя
   * поля вместо объяснения. Ограничение названо числом: «слишком длинное» не говорит, сколько
   * лишнего убрать (журнал 544).
   */
  @MinLength(1, { message: 'Сообщение пустое — напишите текст' })
  @MaxLength(10000, { message: 'Сообщение длиннее 10 000 знаков — сократите текст' })
  textBody!: string;
}
