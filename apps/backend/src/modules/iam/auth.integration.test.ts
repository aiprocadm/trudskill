...
describe('auth integration foundation', () => {
  it('supports logout-all and revokes all active sessions for user', () => {
    ...
    expect(auth.listSessions('tenant_demo', 'u_tenant_admin').filter((item) => !item.revokedAt)).toHaveLength(2);

    auth.logoutAll('tenant_demo', 'u_tenant_admin', context);

    expect(auth.listSessions('tenant_demo', 'u_tenant_admin').every((item) => Boolean(item.revokedAt))).toBe(true);
  });

  it('revokes selected session', () => {
    const audit = new AuditService();
    const iam = new IamService();
    const auth = new AuthService(iam, audit);

    const login = auth.login('tenant_demo', { login: 'tenant_admin', password: 'Password123!' }, context);
    auth.revokeSession('tenant_demo', 'u_tenant_admin', login.sessionId, context);

    expect(auth.listSessions('tenant_demo', 'u_tenant_admin')[0]?.revokedAt).toBeDefined();
  });

  it('does not refresh with token from another tenant', () => {
    const audit = new AuditService();
    const iam = new IamService();
    const auth = new AuthService(iam, audit);
    ...
    expect(() => auth.refresh('tenant_other', login.refreshToken, context)).toThrow(UnauthorizedException);
  });

  it('writes audit event on session revoke', () => {
    const audit = new AuditService();
    const iam = new IamService();
    const auth = new AuthService(iam, audit);
