...
describe('PlatformTenantsController.impersonate (срез 3)', () => {
  ...
  it('ставит refresh и csrf cookie, как обычный логин', async () => {
    ...
    expect(value.some((c) => c.startsWith('cdoprof_refresh_token=refresh_token_value'))).toBe(true);
    expect(value.some((c) => c.startsWith('cdoprof_csrf_token=csrf_token_value'))).toBe(true);
  });
