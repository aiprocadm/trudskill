...
describe('formatMoment', () => {
  it('даёт человеческую дату вместо ISO — во времени ЦЕНТРА', () => {
    // 09:05 UTC — это 12:05 в Москве. Раньше печаталось UTC, и проверяющий читал
    // время, которого на часах не было (журнал 301).
    expect(formatMoment('2026-07-30T09:05:00.000Z')).toBe('30.07.2026 12:05');
    expect(formatMoment('2026-07-30T09:05:00.000Z', 'Asia/Novosibirsk')).toBe('30.07.2026 16:05');
  });

  it('пояс с опечаткой не лишает проверяющего досье', () => {
    expect(formatMoment('2026-07-30T09:05:00.000Z', 'Не/Пояс')).toBe('30.07.2026 12:05');
  });
