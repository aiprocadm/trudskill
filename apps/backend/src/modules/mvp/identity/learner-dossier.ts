 ...
 * личность, какие были экзамены, какие выданы документы, что человек подписал.
 *
 * **Чего в деле пока НЕТ:** IP и устройства экзаменационной сессии. В `TestAttempt` их
 * не существует вовсе — они есть только в журнале аудита, а его чтение сейчас тянет
 * весь тенант без фильтров. Поля в тип НЕ заведены намеренно: всегда пустое поле
 * создаёт ложную уверенность, будто данные собираются.
 *
