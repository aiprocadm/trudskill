...
describe('mvp service domain rules', () => {
  ...
  it('calculates progress based on min_view_seconds and aggregates module/course', () => {
    ...
    const p1 = service.upsertMaterialProgress('tenant_demo', ctx.userId, material.id, { studiedSeconds: 40 }, ctx);
    expect(p1.status).toBe('in_progress');
    expect(p1.progressPercent).toBe(40);

    service.upsertMaterialProgress('tenant_demo', ctx.userId, material.id, { studiedSeconds: 100 }, ctx);
