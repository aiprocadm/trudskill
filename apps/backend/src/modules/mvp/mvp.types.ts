...
export interface Material extends BaseEntity {
  ...
  videoAssetId?: string;
  /**
   * ТЗ 2.5.a: тело текстового материала (`materialType='text'`).
   *
   * Простой текст без разметки. До миграции 0095 хранить его было НЕГДЕ, и экран слушателя
   * печатал ему служебную фразу «станет доступен после расширения backend полем textBody»
   * вместо самого материала — то есть курс из одного текстового урока пройти было нельзя.
   */
  textBody?: string;
  /**
   * ТЗ 2.5.a: адрес внешнего материала (`materialType='external_url'`), например запись вебинара.
   *
   * Просмотрщик ссылки существовал и раньше, но получал жёстко переданный `null` и сообщал
   * «ссылка пока не задана администратором» — неправда: задать её было некуда.
   */
  externalUrl?: string;
}
...
export interface IdentityVerification extends BaseEntity {
  ...
  /**
   * Отпечатки удалённых снимков (ТЗ 17.2, решение Р17).
   *
   * **Зачем.** Р17 требует: снимки удаляются, а ПРОТОКОЛ верификации остаётся — «кто, когда и
   * с каким результатом проверил, плюс хеш файла». Без отпечатка протокол недоказуем: через
   * год, когда файлов давно нет, невозможно подтвердить, что модератор смотрел именно тот
   * документ, который слушатель прислал. Слушатель говорит «я присылал другое» — и возразить
   * нечем (журнал 578).
   *
   * **Почему отпечаток, а не сам файл.** Он не содержит изображения и по нему нельзя
   * восстановить ни лицо, ни номер паспорта: это просто короткая строка, совпадающая у
   * одинаковых файлов. Хранить его безопасно сколько угодно.
   */
  imageHashes?: {
    selfie?: string;
    passport?: string;
...
export interface ProgramMeta {
  ...
  /**
   * Фаза 2 Task 11 (ФТ-E1): строгий порядок модулей. Включено — слушатель не откроет
   * материал модуля, пока не закрыл обязательные материалы всех предыдущих модулей.
   * Выключено по умолчанию: включение задним числом не должно запирать идущие группы.
   * Персистится JSONB-снапшотом (правило 0016), миграция не нужна.
   */
