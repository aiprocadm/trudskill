...
describe('stage13 business e2e flows (service-level)', () => {
  it('completes auth flow: login -> refresh -> logout and blocks blocked user', async () => {
    ...
    await expect(
      auth.login('tenant_demo', { login: 'blocked_user', password: 'Password123!' }, baseCtx)
    ).rejects.toThrow();
  });
  ...
  it('completes learner journey: create course/group -> enroll -> progress -> exam', () => {
    ...
    const course = service.createCourse(
      'tenant_demo',
      baseCtx.userId,
      { code: 'C-E2E-13', title: 'Stage13 Course' },
      baseCtx
    );
    ...
    const module = service.createModule(
      'tenant_demo',
      baseCtx.userId,
      { courseVersionId: version.id, title: 'Module', minViewSeconds: 0 },
      baseCtx
    );
    ...
    const group = service.createGroup(
      'tenant_demo',
      baseCtx.userId,
      { code: 'G-E2E-13', name: 'Stage13 Group' },
      baseCtx
    );
    ...
    const learner = service.createLearner(
      'tenant_demo',
      baseCtx.userId,
      { code: 'L-E2E-13', name: 'Stage13 Learner' },
      baseCtx
    );
    ...
    const bank = service.createQuestionBank(
      'tenant_demo',
      baseCtx.userId,
      { title: 'E2E bank', courseId: course.id },
      baseCtx
    );
  ...
  it('completes document generation and e-sign signing flow with legal log artifacts', () => {
    ...
    const template = documents.createTemplate(
      'tenant_demo',
      baseCtx.userId,
      { name: 'Completion template', templateType: 'certificate' },
      baseCtx
    );
    const version = documents.createTemplateVersion('tenant_demo', baseCtx.userId, {
    ...
    const generated = documents.completeTask(
      'tenant_demo',
      task.id,
      'file_generated_13',
      baseCtx.userId
    ...
    const app = esign.createApplication(
      'tenant_demo',
      baseCtx.userId,
      { learnerId: 'learner_stage13' },
      baseCtx
    );
    const appFile = esign.createApplicationFile('tenant_demo', baseCtx.userId, {
    ...
    esign.startProcess('tenant_demo', 'u_staff_1', process.id, {
