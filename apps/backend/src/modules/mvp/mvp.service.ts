...
/**
 * Сколько записей отдаётся на странице списка по умолчанию (ТЗ 15.3, решение владельца Р16).
 *
 * Двадцать строк означали постоянное пролистывание там, где у центра сотни слушателей.
 * Пятьдесят — компромисс, который владелец и назвал: меньше — листать без конца, больше —
 * заметная задержка отрисовки на слабом телефоне.
 *
 * Совпадение с бюджетом `PERFORMANCE_BUDGETS.listPageSize` стережёт тест: держать это число в
 * двух местах дешевле, чем тащить в бэкенд новую зависимость ради одной константы, но только
 * если расхождение не проходит молча (журнал 585).
 */
 ...
 * разом. Внутренние вызовы передают число и потолка не имеют: госвыгрузки собирают зачисления
...
export class MvpService {
  ...
  updateLearnerExtended(
    ...
    if (request.phone !== undefined) current.phone = request.phone?.trim() || undefined;
    if (request.status !== undefined) current.status = request.status;
  ...
  private list<T extends BaseEntity>(
     ...
     */
    /*
     * ТЗ 15.3 (решение Р16): размер страницы по умолчанию — 50, а не 20. Двадцать строк
     * означают постоянное пролистывание там, где у центра сотни слушателей; пятьдесят — тот
     * компромисс, который владелец и назвал.
     *
     * Число объявлено здесь, а не взято из пакета общих типов: тащить новую зависимость в
     * бэкенд ради одной константы дороже, чем сверить два числа тестом. Расхождение с
     * бюджетом ловит сторож — тот же приём, что у копии разбора тикета (журнал 585).
     */
