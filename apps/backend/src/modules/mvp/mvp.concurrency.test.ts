...
describe('mvp service concurrency-lite invariants', () => {
  it('keeps enrollment unique when duplicate requests race on same group+learner pair', async () => {
    ...
    const group = service.createGroup(
      'tenant_demo',
      ctx.userId,
      { code: 'GR-CNC-1', name: 'Concurrency group' },
      ctx
    );
    const learner = service.createLearner(
      'tenant_demo',
      ctx.userId,
      { code: 'LR-CNC-1', name: 'Concurrent Learner' },
      ctx
    );
  ...
  it('keeps submitAttempt idempotent for duplicated concurrent submissions', async () => {
    ...
    const course = service.createCourse(
      'tenant_demo',
      ctx.userId,
      { code: 'C-CNC-2', title: 'Assessment race' },
      ctx
    );
    const group = service.createGroup(
      'tenant_demo',
    ...
    const learner = service.createLearner(
      'tenant_demo',
      ctx.userId,
      { code: 'L-CNC-2', name: 'Learner race' },
      ctx
    );
    ...
    const bank = service.createQuestionBank(
      'tenant_demo',
      ctx.userId,
      { title: 'Race bank', courseId: course.id },
      ctx
    );
    ...
    const correctOption = service['state'].answerOptions.find(
    ...
    service.saveAttemptAnswer(
      ...
      ctx.userId,
      attempt.id,
      { questionId: question.id, answerOptionIds: [correctOption!.id] },
      ctx
    );
