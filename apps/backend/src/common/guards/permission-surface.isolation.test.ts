...
const WITHOUT_PERMISSION: ReadonlyArray<Allowed> = [
  /*
   * ТЗ 12.2, раздел «Фоновые задачи». Права у ручек нет намеренно: человек видит СВОИ долгие
   * операции, отбор идёт по арендатору из токена и по автору задачи. Право «смотреть свои
   * задачи» означало бы, что кто-то отправит задачу и не узнает её судьбы.
   */
  {
    handler: 'background-tasks/background-tasks.controller.ts::Get ',
    why: 'свои фоновые задачи: отбор по арендатору из токена и по автору задачи, чужих не видно'
  },
  {
    handler: 'background-tasks/background-tasks.controller.ts::Get labels',
    why: 'словарь подписей состояний: данных центра не содержит вовсе, одинаков для всех'
  },
  ...
  /*
   * ТЗ 15.5. Обращение «Сообщить о проблеме». Права нет намеренно: сообщить о поломке должен
   * мочь любой вошедший, включая слушателя. Ограничение по праву превратило бы кнопку в
   * украшение для избранных, а чаще всего не открывается как раз у слушателя. Защищено сессией
   * и ограничением частоты; чужих данных ручка не читает и не отдаёт (журнал 583).
   */
  {
    handler: 'support/support.controller.ts::Post support/problem-reports',
    why: 'обращение о поломке от любого вошедшего; защищено сессией и ограничением частоты'
  },

  /*
   * ТЗ 3.6. Поиск по данным. Права у самой ручки нет намеренно: строка поиска есть в шапке у
   * всех, и право на неё означало бы, что у части людей поиск молча ничего не находит — они
   * решат, что он сломан. Права решают здесь не «пустить ли к ручке», а ЧТО она найдёт:
   * область считается внутри, по правам человека (`searchScope`), и человек без единого права
   * получает пустой ответ. Отбор идёт на сервере до обращения к данным — скрыть строку на
   * экране было бы недостаточно (журнал 590).
   */
  {
    handler: 'mvp/mvp.controller.ts::Get search',
    why: 'поиск доступен всем вошедшим; что он найдёт — решают права внутри, без прав ответ пуст'
  },
  ...
  ...[
    ...
    /*
     * ТЗ 17.1. Журнал входов в СВОЮ учётную запись. Права здесь не нужны намеренно: это
     * единственное место, где человек может заметить, что в его запись заходил кто-то ещё.
     * Ограничение по праву означало бы, что увидеть это может только администратор. Чужие
     * входы остаются в общем журнале аудита, куда пускают по праву.
     */
    ['Get auth/login-history', 'свои входы и свои неудачные попытки; чужих здесь нет'],
    /*
     * ТЗ 9.1. Тикет на подключение к живой трансляции. Права здесь не при чём: тикет берут на
     * СВОЙ канал, и защищает его сама сессия — неавторизованный до ручки не доходит. Что
     * человеку положено слушать, решает служба трансляций при подключении, по своим правилам
     * комнат; выдать тикет на чужой канал она всё равно не даст воспользоваться.
     */
    [
      'Post auth/realtime-ticket',
      'подключение к своему каналу; защищено сессией и правилами комнат'
    ],
