...
const WITHOUT_PERMISSION: ReadonlyArray<Allowed> = [
  ...
  // --- Чтение карточки и оформления своего центра сотрудником.
  {
    handler: 'tenant/tenant.controller.ts::Get me',
    why: 'карточка своего центра: название и статус'
  },
  {
    handler: 'tenant/tenant.controller.ts::Get branding',
    why: 'оформление центра: тема красится каждому вошедшему, запись — под правом'
  },
  {
    handler: 'tenant/tenant.controller.ts::Get identity-settings',
    why: 'настройки подтверждения личности: слушателю нужно понимать требования к себе'
  },
  {
    handler: 'tenant/tenant.controller.ts::Get commission',
    why: 'состав аттестационной комиссии: он и так печатается в протоколах, которые видит слушатель'
  },

  // --- Чтение карточки своего центра. Запись закрыта правом tenant.settings.write (0083).
  {
    handler: 'tenant/tenant.controller.ts::Get settings',
    why: 'чтение настроек СВОЕГО центра любым его сотрудником; запись — под правом'
  },
  {
    handler: 'tenant/tenant.controller.ts::Get requisites',
    why: 'чтение реквизитов СВОЕГО центра: они и так печатаются в выдаваемых документах'
  }
];
