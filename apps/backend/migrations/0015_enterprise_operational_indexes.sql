...
comment on index documents_document_tasks_tenant_status_created_idx is
...
comment on index integrations_sync_jobs_tenant_status_created_idx is
...
comment on index integrations_dead_letters_tenant_status_created_idx is
...
comment on index integrations_sync_jobs_tenant_next_retry_active_idx is
...
create index if not exists integrations_dead_letters_tenant_next_retry_active_idx
  ...
  where status in ('queued', 'retry');
comment on index integrations_dead_letters_tenant_next_retry_active_idx is
...
comment on index integrations_idempotency_keys_tenant_scope_hash_uq is
...
comment on index core_sessions_active_tenant_user_revoked_expires_idx is
...
comment on index core_refresh_tokens_active_tenant_user_revoked_expires_idx is
...
comment on index integrations_webhook_events_tenant_provider_event_uq is
