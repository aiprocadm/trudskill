 ...
 * общего источника (`semanticStatusTone`): одно и то же состояние получало цвет из одного
 ...
 * области.
 */
const SEMANTIC_KEY: Record<AsyncTaskStatus, keyof typeof semanticStatusTone> = {
...
export const AsyncStatusWidget = ({ status }: { status: AsyncTaskStatus }): ReactElement => {
  ...
  /* Тон и слово берутся по ОДНОМУ ключу — иначе они и разъезжаются (журнал 546). */
  const tone = semanticStatusTone[SEMANTIC_KEY[status] ?? 'pending'];
  return (
