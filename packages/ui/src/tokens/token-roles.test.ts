 ...
 *
 * Оговорка: `baseVars` (типографика, отступы) сторож сознательно не трогает — там уже лежат
 * объявленные, но не применённые токены (`--ui-content-max`, веса, межстрочные интервалы),
 * чьи роли назначены ТЗ типографики (`UI-011`/`UI-017`). Удалить их нельзя, применить — работа
 * отдельного среза; расхождение записано в журнал (запись 163).
 */
