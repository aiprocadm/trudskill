...
.ui-chat-layout { display: grid; grid-template-columns: minmax(220px, 280px) 1fr; min-height: calc(100dvh - 64px); align-items: stretch; }
.ui-chat-sidebar { border-right: 1px solid var(--ui-border); padding: 12px; background: var(--ui-surface-muted); overflow: auto; }
.ui-chat-sidebar-title { margin: 0 0 10px; font-size: var(--ui-font-size-md); }
...
.ui-chat-messages { overflow: auto; max-height: calc(100dvh - 200px); }
.ui-chat-msg { margin: 0 0 8px; font-size: var(--ui-font-size-sm); line-height: var(--ui-line-height-normal); }
