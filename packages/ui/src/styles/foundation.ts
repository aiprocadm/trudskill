...
@media (max-width: 480px) {
  /*
   * ТЗ 14.3: кнопки внизу страницы не уезжают под системную полосу телефона.
   *
   * Нижние 34 пикселя экрана у телефонов с «чёрточкой» вместо кнопки «Домой» заняты самой
   * системой. Обычный отступ про них не знает, поэтому последняя кнопка — «Следующий вопрос»
   * на экзамене, «Сохранить» в форме — оказывалась ровно под ней: видно, а нажать нельзя.
   * Поправка на безопасную зону на обычном экране равна нулю, так что на настольном
   * браузере не меняется ничего.
   */
  .ui-page,.ui-page-container { padding: 14px 12px calc(14px + env(safe-area-inset-bottom, 0px)); gap: 14px; }
