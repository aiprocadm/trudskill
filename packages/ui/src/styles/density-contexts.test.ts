 ...
 *
 * Контекст «просторно» (24/32) живёт модификатором `.ui-page--spacious`: один универсальный
 * каркас не может быть одновременно плотным для реестра и просторным для дашборда, поэтому
 * просторность включает сама страница-обзор (`PageContainer spacious`).
 */
...
describe('UI-016 · контекст «дашборд и форма»: просторно', () => {
  it('модификатор просторной страницы держит блоки на xxl', () => {
    expect(someRuleContains('.ui-page--spacious', 'gap: var(--ui-space-xxl)')).toBe(true);
  });

  it('карточка внутри просторной страницы получает внутренний отступ xl', () => {
    expect(
      someRuleContains('.ui-page--spacious .ui-section-card', 'padding: var(--ui-space-xl)')
    ).toBe(true);
  });
});
